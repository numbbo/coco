...
coco_problem_t *
coco_allocate_problem(const size_t number_of_variables, const size_t number_of_objectives,
...
coco_duplicate_problem(coco_problem_t *other) {
  size_t i;
  coco_problem_t *problem;
  problem = coco_allocate_problem(other->number_of_variables, other->number_of_objectives,
...
static void private_tfp_evaluate_constraint(coco_problem_t *self, const double *x, double *y) {
  coco_transform_data_t *data;
  assert(self != NULL);
  assert(self->data != NULL);
  data = self->data;
...
static void private_tfp_recommend_solutions(coco_problem_t *self, const double *x, size_t number_of_solutions) {
  coco_transform_data_t *data;
  assert(self != NULL);
  assert(self->data != NULL);
  data = self->data;
...
static void stacked_problem_evaluate(coco_problem_t *self, const double *x, double *y) {
  ...
  assert(
      coco_get_number_of_objectives(self)
...
static void stacked_problem_evaluate_constraint(coco_problem_t *self, const double *x, double *y) {
  ...
  assert(
      coco_get_number_of_constraints(self)
...
coco_problem_t *coco_stacked_problem_allocate(coco_problem_t *problem1, coco_problem_t *problem2) {
  ...
  const size_t number_of_objectives = coco_get_number_of_objectives(problem1)
      + coco_get_number_of_objectives(problem2);
  const size_t number_of_constraints = coco_get_number_of_constraints(problem1)
      + coco_get_number_of_constraints(problem2);
  size_t i;
  char *s;
