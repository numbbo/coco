...
static void private_evaluate_1u_sr(coco_problem_t *self, const double *x, double *y) {
...
static coco_problem_t *f_1u_sharp_ridge(const size_t number_of_variables) {
  ...
  problem_id_length =
      (size_t)snprintf(NULL, 0, "%s_%02lu", "sharp_ridge", (long)number_of_variables);
  problem->problem_id = coco_allocate_memory(problem_id_length + 1);
  snprintf(problem->problem_id, problem_id_length + 1, "%s_%02lu", "sharp_ridge",
           (long)number_of_variables);

  problem->number_of_variables = number_of_variables;
  problem->number_of_objectives = 1;
  problem->number_of_constraints = 0;
  problem->evaluate_function = private_evaluate_1u_sr;
  ...
  private_evaluate_1u_sr(problem, problem->best_parameter, problem->best_value);
  return problem;
}
