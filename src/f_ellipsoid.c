...
static void private_f_ellipsoid_evaluate(coco_problem_t *self, const double *x, double *y) {
  size_t i = 0;
...
static coco_problem_t *f_ellipsoid(const size_t number_of_variables) {
  ...
  problem->evaluate_function = private_f_ellipsoid_evaluate;
  ...
  private_f_ellipsoid_evaluate(problem, problem->best_parameter, problem->best_value);
