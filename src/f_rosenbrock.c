...
static void private_rosenbrock_evaluate(coco_problem_t *self, const double *x, double *y) {
  size_t i;
...
static coco_problem_t *rosenbrock_problem(const size_t number_of_variables) {
  ...
  problem_id_length =
      (size_t)snprintf(NULL, 0, "%s_%02lu", "rosenbrock", (long)number_of_variables);
  problem->problem_id = (char *)coco_allocate_memory(problem_id_length + 1);
  snprintf(problem->problem_id, problem_id_length + 1, "%s_%02lu", "rosenbrock",
           (long)number_of_variables);

  problem->number_of_variables = number_of_variables;
  problem->number_of_objectives = 1;
  problem->number_of_constraints = 0;
  problem->evaluate_function = private_rosenbrock_evaluate;
  ...
  private_rosenbrock_evaluate(problem, problem->best_parameter, problem->best_value);
  return problem;
}
