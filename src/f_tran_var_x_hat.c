...
static void private_evaluate_tran_var_x_hat(coco_problem_t *self, const double *x, double *y) {
  size_t i;
  _tv_xh_data_t *data;
...
static void private_free_tran_var_x_hat(void *thing) {
...
static coco_problem_t *f_tran_var_x_hat(coco_problem_t *inner_problem, long seed) {
  ...
  self = coco_allocate_transformed_problem(inner_problem, data, private_free_tran_var_x_hat);
  self->evaluate_function = private_evaluate_tran_var_x_hat;
  return self;
}
