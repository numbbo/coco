...
static void private_evaluate_1u_k(coco_problem_t *self, const double *x, double *y) {
  size_t i, j;
  double tmp, tmp2;
...
static coco_problem_t *f_1u_katsuura(const size_t number_of_variables) {
  ...
  problem->evaluate_function = private_evaluate_1u_k;
  ...
  private_evaluate_1u_k(problem, problem->best_parameter, problem->best_value);
  return problem;
}
