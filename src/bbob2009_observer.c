...
coco_problem_t *bbob2009_observer(coco_problem_t *problem, const char *options) {
	...
	coco_join_path(base_path, sizeof(base_path), options, coco_get_id(problem),
			NULL);
	...
	coco_join_path(filename, sizeof(filename), base_path, "bbobexp", NULL);/* TODO: pass the correct path as in the old code*/
