...
static void private_f_bueche_rastrigin_evaluate(coco_problem_t *self, const double *x, double *y) {
  size_t i;
  double tmp = 0., tmp2 = 0.;
...
static coco_problem_t *f_bueche_rastrigin(const size_t number_of_variables) {
  ...
  problem->evaluate_function = private_f_bueche_rastrigin_evaluate;
  ...
  private_f_bueche_rastrigin_evaluate(problem, problem->best_parameter, problem->best_value);
  return problem;
}
