...
static void private_weierstrass_evaluate(coco_problem_t *self, const double *x, double *y) {
  size_t i, j;
...
static coco_problem_t *weierstrass_problem(const size_t number_of_variables) {
  ...
  problem->evaluate_function = private_weierstrass_evaluate;
  problem->data = data;
  ...
  private_weierstrass_evaluate(problem, problem->best_parameter, problem->best_value);
  return problem;
}
