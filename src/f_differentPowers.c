...
static void f_differentPowers_evaluate(numbbo_problem_t *self, double *x, double *y) {
    ...
    for (i = 1; i < self->number_of_parameters; ++i) {
        y[0] += pow(fabs(x[i]), 2 + 4 * (((double)(i - 1))/(self->number_of_parameters - 1)));
    }
    y[0] = sqrt(y[0]);
}

static numbbo_problem_t *differentPowers_problem(const size_t number_of_parameters) {
