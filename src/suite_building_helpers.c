...
/**
 * coco_problem_t *coco_create_incomplete_named_bounded_problem(
 ...
 *
 * Return a coco problem, which needs to be completed by setting
 * ->evaluate_function and/or ->evaluate_constraints and ->best_value
 * e.g. via coco_evaluate(this, problem->best_parameter, problem->best_value).
 * 
 * Ideally, the best_parameter of a "raw" problem is all-zeros. 
 *
 * Example:
 *
 *    coco_problem_t *problem = coco_create_incomplete_named_bounded_problem(
 *                          "sphere_20", "sphere", 20, 1, 0, -5, 5, 0);
 *
 * Details: this is yet a tentative name/interface.
 * FIXME: find a better interface for best_parameter?
 * FIXME: the relation between values of interest and best_parameter_value
 */
coco_problem_t *coco_create_incomplete_named_bounded_problem(
                            ...
                            double best_parameter) {
  size_t i;
  coco_problem_t *problem = coco_allocate_problem(dimension, number_of_objectives, number_of_constraints);
  assert(problem->number_of_objectives == number_of_objectives);
  assert(problem->number_of_constraints == number_of_constraints);
