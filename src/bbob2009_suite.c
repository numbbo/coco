...
static coco_problem_t *bbob2009_problem(int function_id, long dimension_, long instance_id) {
  ...
  if (dimension > MAX_DIM)
    coco_error("bbob2009_suite currently supports dimension up to %ld (%lu given)",
        MAX_DIM, dimension);
  ...
  if (function_id == 1) {
    ...
    for (i = 0; i < dimension; ++i) {
      ...
      for (j = 0; j < dimension; ++j) {
        ...
        for (k = 0; k < dimension; ++k) {
          double exponent = (int)k * 1.0 / ((double)(long)dimension - 1.0);
    ...
    for (i = 0; i < dimension; ++i) {
      ...
      for (j = 0; j < dimension; ++j) {
        ...
        for (k = 0; k < dimension; ++k) {
          ...
          const double exponent = (int)k * 1.0 / ((double)(long)dimension - 1.0);
    ...
    for (i = 0; i < dimension; ++i) {
      ...
      current_row = M + i * dimension;
      for (j = 0; j < dimension; ++j) {
        double exponent = (int)i * 1.0 / ((double)(long)dimension - 1.0);
