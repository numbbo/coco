...
/* --- Changes : ---
  ...
  12/05/28: random seed > 2e9 prohibited to avoid an infinite loop on 32bit
  systems
  ...
  12/10/xx: added some string convenience functions isNoneStr, new_string,
  assign_string
  ...
  14/06/16: test of un-initialized version number removed (thanks to Paul
  Zimmermann)
  14/10/18: splitted cmaes_init() into cmaes_init_para() and cmaes_init_final(),
            ...
            code, namely after calling cmaes_init_para(). The order of
  parameters
...
double *cmaes_PerturbSolutionInto(cmaes_t *t, double *xout, double const *xin,
                                  double eps);
...
static void FATAL(char const *sz1, char const *s2, char const *s3,
                  char const *s4);
static void ERRORMESSAGE(char const *sz1, char const *s2, char const *s3,
                         char const *s4);
...
char *cmaes_SayHello(cmaes_t *t) {
  ...
  sprintf(t->sOutString, "(%d,%d)-CMA-ES(mu_eff=%.1f), Ver=\"%s\", "
                         "dimension=%d, diagonalIterations=%ld, randomSeed=%d "
                         "(%s)",
          t->sp.mu, t->sp.lambda, t->sp.mueff, t->version, t->sp.N,
          (long)t->sp.diagonalCov, t->sp.seed, getTimeStr());
...
void cmaes_init_para(cmaes_t *t, /* "this" */
  ...
  readpara_init(&t->sp, dimension, inxstart, inrgstddev, inseed, lambda,
 ...
 * */
{
  ...
  if (t->version == NULL) {
    ERRORMESSAGE("cmaes_init_final called (probably) without calling "
                 "cmaes_init_para first",
                 ", which will likely lead to unexpected results", 0, 0);
    printf("Warning: cmaes_init_final called (probably) without calling "
           "cmaes_init_para first\n");
  }
  if (strcmp(c_cmaes_version, t->version) != 0) {
    ERRORMESSAGE(
        "cmaes_init_final called twice, which will lead to a memory leak",
        "; use cmaes_exit() first", 0, 0);
    printf("Warning: cmaes_init_final called twice, which will lead to a "
           "memory leak; use cmaes_exit first\n");
  }
  ...
  t->sigma = sqrt(
      trace / N); /* t->sp.mueff/(0.2*t->sp.mueff+sqrt(N)) * sqrt(trace/N); */
  ...
  for (dtest = 1.; dtest && dtest < 1.1 * dtest; dtest *= 2.)
    if (dtest == dtest + 1.)
      break;
  t->dMaxSignifKond = dtest / 1000.; /* not sure whether this is really save,
  ...
  ++t->rgFuncValue;
  t->arFuncValueHist =
      new_double(10 + (int)ceil(3. * 10. * N / t->sp.lambda) + 1);
...
double *cmaes_init(cmaes_t *t, /* "this" */
                   ...
                   long int inseed, int lambda,
                   const char *input_parameter_filename) {
  cmaes_init_para(t, dimension, inxstart, inrgstddev, inseed, lambda,
                  input_parameter_filename);
...
void cmaes_resume_distribution(cmaes_t *t, char *filename) {
  ...
  if (d != t->sp.N)
    FATAL("cmaes_resume_distribution(): Dimension numbers do not match", 0, 0,
          0);
...
double *const *cmaes_SamplePopulation(cmaes_t *t) {
  ...
  for (iNk = 0; iNk < t->sp.lambda;
       ++iNk) { /* generate scaled random vector (D * z)    */
    for (i = 0; i < N; ++i)
      if (flgdiag)
        t->rgrgx[iNk][i] =
            xmean[i] + t->sigma * t->rgD[i] * random_Gauss(&t->rand);
      else
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
double *cmaes_PerturbSolutionInto(cmaes_t *t, double *rgx, double const *xmean,
                                  double eps) {
  int i, j, N = t->sp.N;
  double sum;
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
const double *cmaes_Optimize(cmaes_t *evo,
                             double (*pFun)(double const *, int dim),
                             long iterations)
  ...
  */
  ...
  for (i = 0; i < N; ++i) {
    if (!flgdiag)
      ...
      sum = t->rgdTmp[i];
    t->rgps[i] =
        (1. - t->sp.cs) * t->rgps[i] + sqrt(t->sp.cs * (2. - t->sp.cs)) * sum;
  }
...
static void Adapt_C2(cmaes_t *t, int hsig) {
  ...
  if (t->sp.ccov != 0. && t->flgIniphase == 0) {
    ...
    double ccov1 = douMin(
        t->sp.ccov * (1. / t->sp.mucov) * (flgdiag ? (N + 1.5) / 3. : 1.), 1.);
    double ccovmu = douMin(t->sp.ccov * (1 - 1. / t->sp.mucov) *
                               (flgdiag ? (N + 1.5) / 3. : 1.),
                           1. - ccov1);
  ...
  while (key < keyend) {
    ...
    if (strncmp(key, "B", 1) == 0) {
      ...
      Sorted_index(t->rgD, iindex,
                   N); /* should not be necessary, see end of QLalgo2 */
      /* One eigenvector per row, sorted: largest eigenvalue first */
      for (i = 0; i < N; ++i)
        for (j = 0; j < N; ++j)
          fprintf(fp, "%g%c", t->B[j][iindex[N - 1 - i]],
                  (j == N - 1) ? '\n' : '\t');
      ++key;
      free(iindex); /* MT */
    }
    ...
    if (strncmp(key, "lambda", 6) == 0 || strncmp(key, "popsi", 5) == 0 ||
        strncmp(key, "populationsi", 12) == 0) {
    ...
    if (strncmp(key, "all", 3) == 0) {
      ...
      for (i = 0; i < N; ++i)
        fprintf(fp, " %12g%c", t->rgxmean[i],
                (i % 5 == 4 || i == N - 1) ? '\n' : ' ');
      fprintf(fp,
              "Standard deviation of coordinate axes (sigma*sqrt(diag(C)))\n");
      for (i = 0; i < N; ++i)
...
double cmaes_Get(cmaes_t *t, char const *s) {
  ...
  if (strncmp(s, "axisratio", 5) == 0) { /* between lengths of longest and
    ...
    return (t->gen);
             ...
             strncmp(s, "stopMaxFunEvals", 12) ==
                 0) { /* maximal number of function evaluations */
    return (t->sp.stopMaxFunEvals);
  } else if (strncmp(s, "maxgen", 4) == 0 || strncmp(s, "MaxIter", 7) == 0 ||
             strncmp(s, "stopMaxIter", 11) ==
                 0) { /* maximal number of generations */
    ...
    return (N);
             ...
             strncmp(s, "popsize", 7) ==
                 0) { /* sample size, offspring population size */
    return (t->sp.lambda);
...
const char *cmaes_TestForTermination(cmaes_t *t) {
  ...
  /* TolFun */
  range = douMax(rgdouMax(t->arFuncValueHist,
                          (int)douMin(t->gen, *(t->arFuncValueHist - 1))),
                 rgdouMax(t->rgFuncValue, t->sp.lambda)) -
          douMin(rgdouMin(t->arFuncValueHist,
                          (int)douMin(t->gen, *(t->arFuncValueHist - 1))),
  ...
  /* TolUpX */
  for (i = 0; i < N; ++i) {
    if (t->sigma * sqrt(t->C[i][i]) >
        t->sp.stopTolUpXFactor * t->sp.rgInitialStds[i])
      break;
  }
  if (i < N) {
  ...
  if (t->countevals >= t->sp.stopMaxFunEvals)
    cp +=
        sprintf(cp, "MaxFunEvals: conducted function evaluations %.0f >= %g\n",
...
*/
{
  ...
  int flgprinted = 0;
  int flgwritten = 0;
  double deltaprinttime =
      time(NULL) - t->printtime; /* using clock instead might not be a good */
  double deltawritetime =
      time(NULL) - t->writetime; /* idea as disc time is not CPU time? */
  double deltaprinttimefirst = t->firstprinttime
                                   ? time(NULL) - t->firstprinttime
                                   : 0; /* time is in seconds!? */
  double deltawritetimefirst =
      t->firstwritetime ? time(NULL) - t->firstwritetime : 0;
...
*/
{
  ...
  for (i = 0; i < N; ++i)
    for (j = 0; j < N; ++j) {
      ...
      if (fabs(cc - C[i > j ? i : j][i > j ? j : i]) / sqrt(C[i][i] * C[j][j]) >
              1e-10 &&
...
void cmaes_UpdateEigensystem(cmaes_t *t, int flgforce) {
  ...
  if (flgforce == 0) {
    ...
    if (t->sp.updateCmode.maxtime < 1.00 &&
        t->eigenTimings.tictoctime >
            t->sp.updateCmode.maxtime * t->eigenTimings.totaltime &&
 ...
 */
{
  ...
  if (rgtmp == NULL) /* was OK in former versions */
    FATAL("cmaes_t:Eigen(): input parameter double *rgtmp must be non-NULL", 0,
          0, 0);
...
double timings_update(timings_t *t) {
  ...
  t->lastclock = clock(); /* measures at most 2147 seconds, where 1s = 1e6
                             CLOCKS_PER_SEC */
  t->lasttime = time(NULL);

  diffc = (double)(t->lastclock - lc) /
...
static char *szCat(const char *sz1, const char *sz2, const char *sz3,
                   const char *sz4);

/* --------------------------------------------------------- */
/* -------------- Functions: readpara_t -------------------- */
/* --------------------------------------------------------- */
void readpara_init(readpara_t *t, int dim, const double *inxstart,
                   const double *inrgsigma, int inseed, int lambda,
                   const char *filename) {
  int i, N;
  ...
  t->diagonalCov =
      0; /* default is 0, but this might change in future, see below */
...
void readpara_ReadFromFile(readpara_t *t, const char *filename) {
  ...
  for (ipara = 0; ipara < t->n2para; ++ipara) {
    ...
    {                                       /* skip comments  */
      if (s[0] == '#' || s[0] == '%')
        continue;
      if (sscanf(s, t->rgskeyar[ipara], &size) ==
          1) { /* size==number of values to be read */
        if (size > 0) {
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
void readpara_WriteToFile(readpara_t *t, const char *filenamedest) {
  int ipara, i;
  size_t len;
  time_t ti = time(NULL);
  ...
  for (ipara = 0; ipara < t->n2para; ++ipara) {
    ...
    fprintf(fp, "\n");
    for (i = 0; i < t->N; ++i)
      fprintf(fp, "%7.3g%c", (*t->rgp2adr[ipara])[i],
              (i % 5 == 4) ? '\n' : ' ');
    fprintf(fp, "\n");
  }
...
*/
{
  ...
  t->damps =
      t->damps * (1 +
                  2 * douMax(0., sqrt((t->mueff - 1.) / (N + 1.)) -
                                     1)) /* basic factor */
          *
          douMax(0.3, 1. - /* modify for short runs */
                          (double)N /
                              (1e-6 + douMin(t->stopMaxIter,
                                             t->stopMaxFunEvals / t->lambda))) +
...
static int isNoneStr(const char *filename) {
  if (filename &&
      (strcmp(filename, "no") == 0 || strcmp(filename, "non") == 0 ||
...
/* ========================================================= */
void ERRORMESSAGE(char const *s1, char const *s2, char const *s3,
                  char const *s4) {
  time_t t = time(NULL);
...
/* ========================================================= */
char *szCat(const char *sz1, const char *sz2, const char *sz3,
            const char *sz4) {
  ...
  if (sz2)
    strncat((char *)szBuf, sz2,
            (unsigned)intMin((int)strlen(sz2) + 1,
                             698 - (int)strlen((char const *)szBuf)));
  if (sz3)
    strncat((char *)szBuf, sz3,
            (unsigned)intMin((int)strlen(sz3) + 1,
                             698 - (int)strlen((char const *)szBuf)));
  if (sz4)
    strncat((char *)szBuf, sz4,
            (unsigned)intMin((int)strlen(sz4) + 1,
                             698 - (int)strlen((char const *)szBuf)));
  return (char *)szBuf;
}
