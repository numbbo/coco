...
def load_best_algorithm(best_algo_filename, force=False, relative_load=True):
    ...
    the testbedsettings.current_testbed.best_algorithm_displayname
    ...
    if pickleFilename.endswith('pickle.gz'):
        ...
        try:
            ...
            # filename to set best_algorithm_displayname in testbedsettings
            # if not present:
            if testbedsettings.current_testbed:            
                if hasattr(testbedsettings.current_testbed, 'best_algorithm_displayname'):
                    testbedsettings.current_testbed.best_algorithm_displayname = pickleFilename
        except:
...
def getAllContributingAlgorithmsToBest(algnamelist, target_lb=1e-8,
    ...
    """Computes first the artificial best algorithm from given algorithm list
       ...
       containing all necessary data folders::
        ...
        >>> os.chdir(path)
        >>> bestalg.getAllContributingAlgorithmsToBest(('data/IPOP-CMA-ES.tgz', 'data/RANDOMSEARCH.tgz')) # doctest:+ELLIPSIS
...
#    bestalgentries = generate(dictAlg, dsList[0].algId)
    ...
    bestalgentries = load_best_algorithm(path_to_bestalg_file, relative_load=False, force=True)
