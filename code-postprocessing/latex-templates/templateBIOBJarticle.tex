...
%Tea: Why are there no page numbers? Are they forbidden? I think it would be good to have them...
...
%Tea: What about the deadline extension?
...
%Tea: We should either change the output of timing of the example experiment from seconds to milliseconds
%Tea: or have seconds instead of milliseconds here (the latter one is easier). 
...
%Tea: I have a problem with these captions, or rather, with the fact that the first one is different 
%Tea: than the others (the first mentions bootstrapping and the others not, the others mention 
%Tea: "of the restarted algorithm", which is I believe redundant here, and the first not). To me it 
%Tea: would make much more sense to write either the same thing everywhere or have more text in the
%Tea: first caption, but then not introduce any previously unmentioned thing in the other captions. 
%Tea: Also, depending on what will be the final caption, the set of target precisions should be split 
%Tea: so that the text is not so stretched (see Figs. 1 & 4). 
%Tea: Additionally, instead of "for 33 targets in" the text should say "for 33 targets with target 
%Tea: precision in"
...
%Tea: I would reformulate the caption as follows: 
%Tea: Average running time (\aRT) to reach the given targets measured in number of 
%Tea: $f$-evaluations. For each function, the \aRT and in braces, as dispersion measure, the half 
%Tea: difference between 90\%- and 10\%-tile of bootstrapped run lengths are shown for the 
%Tea: different target values ($\fopt + \Df$). \#succ is the number of trials that reached 
%Tea: the final target ($\fopt +10^{-5}$). If the final target was never reached, the median 
%Tea: number of conducted $f$-evaluations is additionally given in \textit{italics}. 
}
\end{table*}

%Tea: Shouldn't the table be placed after Fig. 4?
...
%Tea: I would reformulate the caption as follows: 
%Tea: Scaling of average running time (\aRT, measured with number of $f$-evaluations) 
%Tea: with dimension: \aRT to reach the target (lines); median number of $f$-evaluations 
%Tea: to reach the final target that was reached at least once but not always (+); maximum 
%Tea: number of $f$-evaluations in any trial ({\color{red}$\times$}); interquartile 
%Tea: range with median of simulated run-lengths to reach the target  (notched boxes).  
%Tea: All values are divided by dimension and plotted in log-scale. Colors 
%Tea: represent different targets $\fopt + \Df$, where $\Df = 10^{\{-1, -2, -3, -4, -5\}}$.  
%Tea: Numbers above \aRT-symbols (if appearing) indicate the number of trials
%Tea: reaching the respective target. Horizontal lines mean linear scaling, slanted
%Tea: grid lines depict quadratic scaling.  
}
\end{figure*}

%Tea: On paper, the red crosses can hardly be seen.
