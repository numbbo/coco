...
\item Collection of run-length for a parametrized function for a set of instances: Given a parametrized function $\x \to \mathcal{F}_{\theta}(\x)$ and a set of instances $(\thetabar_1, \ldots,\thetabar_K)$, we compute the instance-wise targets associated to a given precision (see above). We then collect the runtime associated to those targets. \anne{We might want to give some names here ...}
