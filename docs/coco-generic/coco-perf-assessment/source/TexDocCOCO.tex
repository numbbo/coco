...
\item We can aggregate several run-lengths in which case we have to selecting a subset of problems. We can vary $f_\target$ only, or vary $\theta$ only, or we can vary both and/or additionally the function. If we vary $f_\target$ only, the events are not independent. 
