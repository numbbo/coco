...
static coco_suite_t *suite_cons_bbob_initialize(const char *suite_name) {
   ...
   */
  suite = coco_suite_allocate(suite_name, 54, num_dimensions, dimensions,
                              "year: 2022");
...
static const char *suite_cons_bbob_get_instances_by_year(const int year) {
  ...
  if ((year >= 2022) || (year == 0)) {
    ...
    coco_error("suite_cons_bbob_get_instances_by_year(): year %d not defined "
               "for suite_cons_bbob",
               year);
    return NULL;
  }
...
static coco_problem_t *coco_get_cons_bbob_problem(const char *suite_name,
  ...
  const char *problem_name_template =
      "bbob-constrained suite problem f%lu instance %lu in %luD";
  if (strcmp(suite_name, "bbob-constrained-active-only") == 0) {
    /* CAVEAT: the first 28 chars of this ID are used in
     * c_linear_cons_bbob_problem_allocate() in c_linear.c to make the decision
     * whether inactive constraints are added */
    problem_id_template = "bbob-constrained-active-only_f%03lu_i%02lu_d%02lu";
    problem_name_template =
        "bbob-constrained-active-only suite problem f%lu instance %lu in %luD";
  }
  if (strcmp(suite_name, "bbob-constrained-no-disguise") == 0) {
    ...
    problem_name_template =
        "bbob-constrained-no-disguise suite problem f%lu instance %lu in %luD";
  }
  ...
  if (obj_function_type(function) == 1) {
    ...
    problem = f_sphere_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 10.); /* move initial feasible point to a
                                              delta-f between 100 and 10000 */

  } else if (obj_function_type(function) == 2) {

    problem = f_ellipsoid_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 1e-4); /* move initial feasible point to a
                                               delta-f between 100 and 10000 */

  } else if (obj_function_type(function) == 3) {

    problem = f_linear_slope_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 10.); /* move initial feasible point to a
    ...
    problem = f_ellipsoid_rotated_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 1e-4); /* move initial feasible point to a
    ...
    problem = f_discus_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 1e-4); /* move initial feasible point to a
    ...
    problem = f_bent_cigar_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem = transform_obj_scale(problem,
                                  1e-4); /* move initial feasible point to a
                                            delta-f between 100 and 100,000 */

  } else if (obj_function_type(function) == 7) {

    problem = f_different_powers_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 1e2); /* move initial feasible point to a
                                              delta-f between 100 and 1000 */

  } else if (obj_function_type(function) == 8) {

    problem = f_rastrigin_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 10.); /* move initial feasible point to a
    ...
    problem = f_rastrigin_rotated_c_linear_cons_bbob_problem_allocate(
        function, dimension, instance, number_of_linear_constraints, rseed,
        feasible_direction, xopt, problem_id_template, problem_name_template);
    problem =
        transform_obj_scale(problem, 10.); /* move initial feasible point to a
    ...
    coco_error("get_cons_bbob_problem(): cannot retrieve problem f%lu instance "
               "%lu in %luD",
...
static coco_problem_t *suite_cons_bbob_get_problem(coco_suite_t *suite,
  ...
  problem = coco_get_cons_bbob_problem(suite->suite_name, function, dimension,
                                       instance);
