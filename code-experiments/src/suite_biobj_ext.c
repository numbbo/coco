 ...
 * @brief Implementation of the extended biobjective bbob-biobj-ext suite
 * containing 92 functions and 6 dimensions.
 *
 * The bbob-biobj-ext suite is created by combining two single-objective
 * problems from the bbob suite. The first 55 functions are the same as in the
 * original bbob-biobj test suite to which 37 functions are added. Those
 * additional functions are constructed by combining all not yet contained
 * in-group combinations (i,j) of single-objective bbob functions i and j such
 * that i<j (i.e. in particular not all combinations (i,i) are included in this
 * bbob-biobj-ext suite), with the exception of the Weierstrass function (f16)
 * for which the optimum is not unique and thus a nadir point is difficult to
 * compute, see http://numbbo.github.io/coco-doc/bbob-biobj/functions/ for
 * details.
 *
 ...
 * suite_biobj_ext_instances. If a new instance of the problem is called, a
 * check ensures that the two underlying single-objective instances create a
 * true bi-objective problem. However, these new instances need to be manually
 * added to suite_biobj_ext_instances, otherwise they will be computed each time
 * the suite constructor is invoked with these instances.
 *
 * @note This file is based on the original suite_bbob_biobj.c and extends it by
 * 37 functions in 6 dimensions.
 */
...
typedef struct {
  ...
  size_t **new_instances; /**< @brief A matrix of new instances (equal in form
                             to suite_biobj_ext_instances) that needs to be used
...
/**
 * @brief Sets the dimensions and default instances for the bbob-biobj-ext
 * suite.
 */
static coco_suite_t *suite_biobj_ext_initialize(void) {
   ...
   */
  suite = coco_suite_allocate("bbob-biobj-ext", 55 + 37, num_dimensions,
                              dimensions, "year: 2018");
...
static const char *suite_biobj_ext_get_instances_by_year(const int year) {
  ...
  if (year == 0000) { /* default/test case */
    ...
    coco_error("suite_biobj_ext_get_instances_by_year(): year %d not defined "
               "for suite_biobj_ext",
               year);
    return NULL;
  }
...
/**
 * @brief Returns the problem from the bbob-biobj-ext suite that corresponds to
 * the given parameters.
 *
 * Creates the bi-objective problem by constructing it from two single-objective
 * problems from the bbob suite. If the invoked instance number is not in
 * suite_biobj_ext_instances, the function uses the following formula to
 * construct a new appropriate instance:
 *
 *   problem1_instance = 2 * biobj_instance + 1
 *
 *   problem2_instance = problem1_instance + 1
 *
 * If needed, problem2_instance is increased (see also the explanation of
...
static coco_problem_t *suite_biobj_ext_get_problem(coco_suite_t *suite,
  ...
  /* Functions from the bbob suite that are used to construct the original
   * bbob-biobj suite. */
  ...
  const size_t num_existing_instances =
      sizeof(suite_biobj_ext_instances) / sizeof(suite_biobj_ext_instances[0]);
  ...
  if (!instance_found) {
    ...
    if (!data) {
       ...
       * suite_biobj_ext_instances. However, in order to avoid iterating over
       * suite_biobj_ext_instances, the allocation uses max_new_instances. */
      data->max_new_instances = suite->number_of_instances;

      data->new_instances = (size_t **)coco_allocate_memory(
          data->max_new_instances * sizeof(size_t *));
      for (i = 0; i < data->max_new_instances; i++) {
    ...
    /* A simple formula to set the first instance, but instead of for 10
     * functions as in bbob-biobj suite, now for all combinations of functions.
     */
    instance1 = 2 * instance + 1;
    instance2 = suite_biobj_ext_get_new_instance(suite, instance, instance1, 24,
                                                 all_bbob_functions);
  }

  if (function_idx < 55) {
    problem1 = coco_get_bbob_problem(bbob_functions[function1_idx], dimension,
                                     instance1);
    problem2 = coco_get_bbob_problem(bbob_functions[function2_idx], dimension,
                                     instance2);
  } else {
    problem1 = coco_get_bbob_problem(all_bbob_functions[function1_idx],
                                     dimension, instance1);
    problem2 = coco_get_bbob_problem(all_bbob_functions[function2_idx],
                                     dimension, instance2);
  }
...
static int check_consistency_of_instances(const size_t dimension,
                                          size_t function1, size_t instance1,
  ...
  if (norm < 1e-1) { /* TODO How to set this value in a sensible manner? */
    coco_debug("suite_biobj_ext_get_new_instance(): The ideal and nadir points "
               "of %s are too close in the objective space",
               problem->problem_id);
  ...
  if (norm < apart_enough) {
    coco_debug("suite_biobj_ext_get_new_instance(): The extreme points of %s "
...
static size_t suite_biobj_ext_get_new_instance(coco_suite_t *suite,
  ...
  while ((!appropriate_instance_found) && (num_tries < max_tries)) {
      ...
      for (f2 = f1 + 1; (f2 < num_bbob_functions) && !break_search; f2++) {
        ...
        for (d = 0; (d < suite->number_of_dimensions) && !break_search; d++) {
          ...
          if (dimension == 0) {
            if (!warning_produced)
              coco_warning(
                  "suite_biobj_ext_get_new_instance(): remove filtering of "
                  "dimensions to get generally acceptable instances!");
            warning_produced = 1;
            continue;
          }

          break_search = check_consistency_of_instances(
    ...
    if (break_search) {
      ...
      coco_info("suite_biobj_set_new_instance(): Instance %lu created from "
                "instances %lu and %lu",
  ...
  if (!appropriate_instance_found) {
    coco_error("suite_biobj_ext_get_new_instance(): Could not find suitable "
