 ...
 * COCO structures.
 */
static double f_griewank_rosenbrock_raw(const double *x,
...
f_griewank_rosenbrock_allocate(const size_t number_of_variables) {

  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "Griewank Rosenbrock function", f_griewank_rosenbrock_evaluate, NULL,
      number_of_variables, -5.0, 5.0, 1);
  coco_problem_set_id(problem, "%s_d%02lu", "griewank_rosenbrock",
                      number_of_variables);
 ...
 */
static coco_problem_t *f_griewank_rosenbrock_bbob_problem_allocate(
  ...
  for (i = 0; i < dimension; i++) {
    problem->best_parameter[i] =
        0; /* Wassim: TODO: not a proper way of avoiding to trigger
              coco_warning("transform_vars_affine(): 'best_parameter' not
              updated, set to NAN")*/
  }
...
f_griewank_rosenbrock_permblockdiag_bbob_bbob_problem_allocate(
  ...
  for (k = 0; k < nb_blocks; ++k) {
    for (j = 0; j < block_sizes[k];
         ++j) { /* Manh: firstly, set xopt_1 =
                   (B^T)*(P_2^T)*ones(dimension)/(2*scales) */
      tmp = 0;
