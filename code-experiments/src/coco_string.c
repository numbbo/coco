...
static char *coco_vstrdupf(const char *str, va_list args) {
  ...
  if (written > COCO_VSTRDUPF_BUFLEN - 3)
    coco_error("coco_vstrdupf(): A suspiciously long string is tried to being "
               "duplicated '%s'",
               buf);
 ...
 *
 * @note If there is an equivalent standard C function, this can/should be
 * removed.
 */
static long coco_strfind(const char *base, const char *seq) {
  ...
  for (i = 0; i <= last_first_idx; ++i) {
    if (base[i] == seq[0]) {
      ...
      if (j == strlen_seq) {
        if (i > 1000000000UL)
          coco_error("coco_strfind(): strange values observed i=%lu, j=%lu, "
                     "strlen(base)=%lu",
                     (unsigned long)i, (unsigned long)j,
                     (unsigned long)strlen(base));
        return (long)i;
      }
    }
  }
...
static char **coco_string_split(const char *string, const char delimiter) {
   ...
   * NOTE: strtok() ignores multiple delimiters, therefore the final number of
   * detected substrings might be lower than the count. This is OK. */
  i = 0;
...
/**
 * @brief Creates and returns a string with removed characters between from and
 * to.
 *
 * If you wish to remove characters from the beginning of the string, set from
 * to "". If you wish to remove characters until the end of the string, set to
 * to "".
 *
 * The caller is responsible for freeing the allocated memory using
 * coco_free_memory().
 */
static char *coco_remove_from_string(const char *string, const char *from,
                                     const char *to) {
  ...
  if ((start == NULL) || (stop == NULL) || (stop < start)) {
    coco_error("coco_remove_from_string(): failed to remove characters between "
               "%s and %s from string %s",
               from, to, string);
 ...
 *
 * Reads ranges from a string of positive ranges separated by commas. For
 * example: "-3,5-6,8-". Returns the numbers that are defined by the ranges if
 * min and max are used as their extremes. If the ranges with open beginning/end
 * are not allowed, use 0 as min/max. The returned string has an appended 0 to
 * mark its end. A maximum of max_count values is returned. If there is a
 * problem with one of the ranges, the parsing stops and the current result is
 * returned. The memory of the returned object needs to be freed by the caller.
 */
static size_t *coco_string_parse_ranges(const char *string, const size_t min,
                                        const size_t max, const char *name,
  ...
  while (*ptr != '\0') {
    if ((*ptr != '-') && (*ptr != ',') && !isdigit((unsigned char)*ptr)) {
      coco_warning("coco_string_parse_ranges(): problem parsing '%s' - cannot "
                   "parse ranges with '%c'",
                   str, *ptr);
      coco_free_memory(str);
      return NULL;
  ...
  if (ranges) {
    ...
    for (i = 0; *(ranges + i); i++) {
      ...
      if (count > 1) {
        coco_warning(
            "coco_string_parse_ranges(): problem parsing '%s' - too many '-'s",
            string);
        /* Cleanup */
        for (j = i; *(ranges + j); j++)
        ...
        if (j == 0) {
          ...
          if ((min == 0) || (max == 0)) {
            coco_warning("coco_string_parse_ranges(): '%s' ranges cannot have "
                         "an open ends; some ranges ignored",
                         name);
            /* Cleanup */
            for (j = i; *(ranges + j); j++)
              coco_free_memory(*(ranges + j));
          ...
          if (dash - *(ranges + i) == 0) {
            ...
            if (min == 0) {
              coco_warning("coco_string_parse_ranges(): '%s' ranges cannot "
                           "have an open beginning; some ranges ignored",
                           name);
              /* Cleanup */
              for (j = i; *(ranges + j); j++)
                coco_free_memory(*(ranges + j));
            ...
            if (max == 0) {
              coco_warning("coco_string_parse_ranges(): '%s' ranges cannot "
                           "have an open end; some ranges ignored",
                           name);
              /* Cleanup */
              for (j = i; *(ranges + j); j++)
                coco_free_memory(*(ranges + j));
      ...
      if ((min > 0) && (num[0] < min)) {
        num[0] = min;
        coco_warning(
            "coco_string_parse_ranges(): '%s' ranges adjusted to be >= %lu",
            name, (unsigned long)min);
      }
      if ((max > 0) && (num[1] > max)) {
        num[1] = max;
        coco_warning(
            "coco_string_parse_ranges(): '%s' ranges adjusted to be <= %lu",
            name, (unsigned long)max);
      }
      if (num[0] > num[1]) {
        coco_warning("coco_string_parse_ranges(): '%s' ranges not within "
                     "boundaries; some ranges ignored",
                     name);
        /* Cleanup */
        for (j = i; *(ranges + j); j++)
          coco_free_memory(*(ranges + j));
 ...
 *
 * If the string contains any leading spaces, the contents are shifted so that
 * if it was dynamically allocated, it can be still freed on the returned
 * pointer.
 */
static char *coco_string_trim(char *string) {
  ...
  /* Shift the string. Note the reuse of endp to mean the front of the string
   * buffer now. */
  endp = string;
  if (frontp != string) {
    while (*frontp) {
