 ...
 * @brief Implementation of normalizing the raw fitness functions by the
 * dimensions Mostly used to in the large-scale testsuite
 */
...
static void transform_obj_norm_by_dim_evaluate(coco_problem_t *problem,
  ...
  /* Wassim: might want to use a function (with no 40) here that we can put in a
   * helpers file */
...
transform_obj_norm_by_dim(coco_problem_t *inner_problem) {
  coco_problem_t *problem;

  problem = coco_problem_transformed_allocate(inner_problem, NULL, NULL,
                                              "transform_obj_norm_by_dim");
