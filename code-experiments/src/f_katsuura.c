...
static double f_katsuura_raw(const double *x,
  ...
  /*result = 10. / ((double) number_of_variables) / ((double)
   * number_of_variables)
...
static coco_problem_t *f_katsuura_allocate(const size_t number_of_variables) {
  ...
  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "Katsuura function", f_katsuura_evaluate, NULL, number_of_variables, -5.0,
      5.0, 0.0);
...
f_katsuura_bbob_problem_allocate(const size_t function, const size_t dimension,
 ...
 */
static coco_problem_t *f_katsuura_permblockdiag_bbob_problem_allocate(
  ...
  /*problem = transform_obj_norm_by_dim(problem);*/ /* Wassim: does not seem to
                                                       be needed*/
  problem = transform_obj_penalize(problem, penalty_factor);
  problem = transform_obj_shift(problem,
                                fopt); /*TODO: documentation, there is no fopt
