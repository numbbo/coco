...
static coco_problem_t *f_katsuura_allocate(const size_t number_of_variables) {
  ...
  coco_problem_set_id(problem, "%s_d%04lu", "katsuura", number_of_variables);
...
static void deprecated__f_katsuura_evaluate(coco_problem_t *self, const double *x, double *y) {
  size_t i, j;
  double tmp, tmp2;
  assert(self->number_of_objectives == 1);
...
static coco_problem_t *deprecated__f_katsuura(const size_t number_of_variables) {
  ...
  problem->evaluate_function = deprecated__f_katsuura_evaluate;
  for (i = 0; i < number_of_variables; ++i) {
  ...
  deprecated__f_katsuura_evaluate(problem, problem->best_parameter, problem->best_value);
  return problem;
}
