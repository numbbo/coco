 ...
 * @brief Implementation of performing an asymmetric transformation on decision
 * values.
 * @author ??
 * @author Paul Dufosse
...
static double tasy_uv(double xi, tasy_data *d) {
  ...
  if (xi > 0.0) {
    exponent =
        1.0 + ((d->beta * (double)(long)d->i) / ((double)(long)d->n - 1.0)) *
                  sqrt(xi);
    yi = pow(xi, exponent);
  } else {
    yi = xi;
...
/**
 * @brief Inverse of asymmetric non-linear transformation tasy_uv obtained with
 * brentq.
 */
 ...
 */
static void tasy(transform_vars_asymmetric_data_t *data, const double *x,
...
static void transform_vars_asymmetric_evaluate_function(coco_problem_t *problem,
  ...
  data = (transform_vars_asymmetric_data_t *)coco_problem_transformed_get_data(
      problem);
 ...
 */
static void
transform_vars_asymmetric_evaluate_constraint(coco_problem_t *problem,
...
static void transform_vars_asymmetric_free(void *thing) {
  transform_vars_asymmetric_data_t *data =
      (transform_vars_asymmetric_data_t *)thing;
...
static coco_problem_t *transform_vars_asymmetric(coco_problem_t *inner_problem,
  ...
  data =
      (transform_vars_asymmetric_data_t *)coco_allocate_memory(sizeof(*data));
  data->x = coco_allocate_vector(inner_problem->number_of_variables);
  data->beta = beta;
  problem = coco_problem_transformed_allocate(inner_problem, data,
                                              transform_vars_asymmetric_free,
  ...
  if (inner_problem->number_of_constraints > 0)
    problem->evaluate_constraint =
        transform_vars_asymmetric_evaluate_constraint;

  if (inner_problem->number_of_objectives > 0 &&
      coco_problem_best_parameter_not_zero(inner_problem)) {
    coco_warning("transform_vars_asymmetric(): 'best_parameter' not updated, "
                 "set to NAN");
...
/**
 * @brief Applies the inverse of the asymmetric transformation tasy to the
 * initial solution.
 *
 ...
 */
static void transform_inv_initial_asymmetric(coco_problem_t *problem,
                                             const double *xopt) {
  size_t i;
  size_t j;
  int is_in_bounds;
  ...
  while (1) {
    ...
    for (i = 0; i < problem->number_of_variables; ++i) {
      d->i = i;
      di = tasy_uv_inv(problem->initial_solution[i] *
                           pow(halving_factor, (double)(long)j),
                       d);
      xi = di + xopt[i];
