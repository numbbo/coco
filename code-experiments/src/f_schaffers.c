...
static double f_schaffers_raw(const double *x,
  ...
  for (i = 0; i < number_of_variables - 1; ++i) {
    ...
    if (coco_is_inf(tmp) &&
        coco_is_nan(sin(50.0 * pow(tmp, 0.1)))) /* sin(inf) -> nan */
      /* the second condition is necessary to pass the integration tests under
       * Windows and Linux */
      return tmp;
...
static coco_problem_t *f_schaffers_allocate(const size_t number_of_variables) {
  ...
  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "Schaffer's function", f_schaffers_evaluate, NULL, number_of_variables,
      -5.0, 5.0, 0.0);
 ...
 */
static coco_problem_t *f_schaffers_bbob_problem_allocate(
    const size_t function, const size_t dimension, const size_t instance,
    const long rseed, const double conditioning,
 ...
 */
static coco_problem_t *f_schaffers_permblockdiag_bbob_problem_allocate(
    const size_t function, const size_t dimension, const size_t instance,
    const long rseed, const double conditioning,
    const char *problem_id_template, const char *problem_name_template) {
  double *xopt, fopt;
