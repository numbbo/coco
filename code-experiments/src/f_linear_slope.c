...
static void f_linear_slope_evaluate_gradient(coco_problem_t *problem,
  ...
  for (i = 0; i < problem->number_of_variables; ++i) {
    base = sqrt(alpha);
    exponent =
        (double)(long)i / ((double)(long)problem->number_of_variables - 1);
...
static coco_problem_t *f_linear_slope_allocate(const size_t number_of_variables,
  ...
  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "linear slope function", f_linear_slope_evaluate, NULL,
  ...
  f_linear_slope_evaluate(problem, problem->best_parameter,
                          problem->best_value);
 ...
 */
static coco_problem_t *f_linear_slope_bbob_problem_allocate(
