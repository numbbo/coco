 ...
 */
static void
f_sharp_ridge_generalized_versatile_data_free(coco_problem_t *problem) {
...
f_sharp_ridge_generalized_raw(const double *x, const size_t number_of_variables,
                              f_sharp_ridge_generalized_versatile_data_t
                                  *f_sharp_ridge_generalized_versatile_data) {
...
static void f_sharp_ridge_generalized_evaluate(coco_problem_t *problem,
  ...
  y[0] = f_sharp_ridge_generalized_raw(
...
f_sharp_ridge_generalized_allocate(const size_t number_of_variables,
                                   size_t proportion_of_linear_dims) {
  /* Wassim: proportion_of_linear_dims should probably be allowed to be
   * non-integer */
  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "sharp ridge function", f_sharp_ridge_generalized_evaluate,
      f_sharp_ridge_generalized_versatile_data_free, number_of_variables, -5.0,
      5.0, 0.0);

  coco_problem_set_id(problem, "%s_d%02lu", "sharp_ridge_generalized",
                      number_of_variables);
  problem->versatile_data =
      (f_sharp_ridge_generalized_versatile_data_t *)coco_allocate_memory(
          sizeof(f_sharp_ridge_generalized_versatile_data_t));
  ((f_sharp_ridge_generalized_versatile_data_t *)problem->versatile_data)
      ->proportion_of_linear_dims = proportion_of_linear_dims;
...
f_sharp_ridge_generalized_permblockdiag_bbob_problem_allocate(
  ...
  coco_compute_truncated_uniform_swap_permutation(
      P11, rseed + 3000000, dimension, nb_swaps, swap_range);
  coco_compute_truncated_uniform_swap_permutation(
      P21, rseed + 4000000, dimension, nb_swaps, swap_range);
  coco_compute_truncated_uniform_swap_permutation(
      P12, rseed + 5000000, dimension, nb_swaps, swap_range);
  coco_compute_truncated_uniform_swap_permutation(
      P22, rseed + 6000000, dimension, nb_swaps, swap_range);

  problem =
      f_sharp_ridge_generalized_allocate(dimension, proportion_of_linear_dims);
  ...
  problem = transform_vars_permutation(
      problem, P22,
      dimension); /*Consider replacing P11 and 22 by a single permutation P3*/
