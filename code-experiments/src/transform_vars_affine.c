 ...
 * @brief Implementation of performing an affine transformation on decision
 * values.
 *
 * x |-> Mx + b <br>
...
static void transform_vars_affine_evaluate_function(coco_problem_t *problem,
  ...
  data = (transform_vars_affine_data_t *)coco_problem_transformed_get_data(
      problem);
  inner_problem = coco_problem_transformed_get_inner_problem(problem);

  for (i = 0; i < inner_problem->number_of_variables; ++i) {
    /* data->M has problem->number_of_variables columns and
     * inner_problem->number_of_variables rows. */
...
static coco_problem_t *transform_vars_affine(coco_problem_t *inner_problem,
  ...
  problem = coco_problem_transformed_allocate(
      inner_problem, data, transform_vars_affine_free, "transform_vars_affine");
  ...
  /* Update the best parameter by computing
     ...
     The update takes place only if the best parameter or b are different than
     zero and the transformation matrix M is orthogonal.
  */
  if (coco_problem_best_parameter_not_zero(inner_problem) ||
      !coco_vector_is_zero(data->b, inner_problem->number_of_variables)) {
    if (!coco_is_orthogonal(data->M, problem->number_of_variables,
                            inner_problem->number_of_variables))
      coco_warning("transform_vars_affine(): rotation matrix is not "
                   "orthogonal. Best parameter not updated");
    else {
      for (i = 0; i < inner_problem->number_of_variables; ++i) {
      ...
      for (i = 0; i < problem->number_of_variables; ++i) {
        ...
        for (j = 0; j < inner_problem->number_of_variables; ++j) {
          problem->best_parameter[i] +=
              data->M[j * problem->number_of_variables + i] * data->x[j];
        }
      }
    }
  }
