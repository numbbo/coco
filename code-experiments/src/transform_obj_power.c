 ...
 * @brief Implementation of raising the objective value to the power of a given
 * exponent.
 */
...
static void transform_obj_power_evaluate(coco_problem_t *problem,
  ...
  data =
      (transform_obj_power_data_t *)coco_problem_transformed_get_data(problem);
...
static coco_problem_t *transform_obj_power(coco_problem_t *inner_problem,
  ...
  /* Compute best value */
  transform_obj_power_evaluate(problem, problem->best_parameter,
                               problem->best_value);
  return problem;
}
