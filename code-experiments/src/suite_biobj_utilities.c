 ...
 * @brief Implementation of some functions (mostly handling instances) used by
 * the bi-objective suites.
 *
 * @note Because some bi-objective problems constructed from two
 * single-objective ones have a single optimal value, some care must be taken
 * when selecting the instances. The already verified instances are stored in
 * suite_biobj_instances. If a new instance of the problem is called, a check
 * ensures that the two underlying single-objective instances create a true
 * bi-objective problem. However, these new instances need to be manually added
 * to suite_biobj_instances, otherwise they will be computed each time the suite
 * constructor is invoked with these instances.
 */
...
/**
 * @brief The array of triples biobj_instance - problem1_instance -
 * problem2_instance connecting bi-objective suite instances to the instances of
 * the bbob suite.
 *
 * It should be updated with new instances when/if they are chosen.
 */
static const size_t suite_biobj_instances[][3] = {
    {1, 2, 4},    {2, 3, 5},    {3, 7, 8},    {4, 9, 10},   {5, 11, 12},
...
typedef struct {
  ...
  size_t **new_instances; /**< @brief A matrix of new instances (equal in form
                             to suite_biobj_instances) that needs to be used
                             only when an instance that is not in
...
/**
 * @brief  Performs a few checks and returns whether the two given problem
 * instances should break the search for new instances in
 * suite_biobj_get_new_instance().
 */
static int suite_biobj_check_inst_consistency(const size_t dimension,
  ...
  /* Set smallest and largest values of interest to some value (not important
   * which, it just needs to be a vector of doubles of the right dimension) */
  smallest_values_of_interest =
      coco_allocate_vector_with_value(dimension, -100);
  ...
  /* Check whether the ideal and nadir points are too close in the objective
   * space */
  norm = mo_get_norm(problem->best_value, problem->nadir_value, 2);
  if (norm < 1e-1) { /* TODO How to set this value in a sensible manner? */
    coco_debug("suite_biobj_check_inst_consistency(): The ideal and nadir "
               "points of %s are too close in the objective space",
               problem->problem_id);
    coco_debug("norm = %e, ideal = %e\t%e, nadir = %e\t%e", norm,
               problem->best_value[0], problem->best_value[1],
               problem->nadir_value[0], problem->nadir_value[1]);
    break_search = 1;
  }

  /* Check whether the extreme optimal points are too close in the decision
   * space */
  norm = mo_get_norm(problem1->best_parameter, problem2->best_parameter,
                     problem->number_of_variables);
  if (norm < apart_enough) {
    coco_debug("suite_biobj_check_inst_consistency(): The extreme points of %s "
               "are too close in the decision space",
               problem->problem_id);
...
/**
 * @brief Computes the instance number of the second problem/objective so that
 * the resulting bi-objective problem has more than a single optimal solution.
 *
 * Starts by setting instance2 = instance1 + 1 and increases this number until
 * an appropriate instance has been found (or until a maximum number of tries
 * has been reached, in which case it throws a coco_error). An appropriate
 * instance is the one for which the resulting bi-objective problem (in any
 * considered dimension) has the ideal and nadir points apart enough in the
 * objective space and the extreme optimal points apart enough in the decision
 * space. When the instance has been found, it is output through coco_warning,
 * so that the user can see it and eventually manually add it to
 * suite_biobj_instances.
 */
static size_t suite_biobj_get_new_instance(
    suite_biobj_new_inst_t *new_inst_data, const size_t instance,
  ...
  while ((!appropriate_instance_found) && (num_tries < max_tries)) {
    ...
    /* An instance is "appropriate" if the ideal and nadir points in the
     * objective space and the two extreme optimal points in the decisions space
     * are apart enough for all problems (all dimensions and function
     * combinations); therefore iterate over all dimensions and function
     * combinations */
      ...
      for (f2 = f1 + 1; (f2 < num_bbob_functions) && !break_search; f2++) {
        ...
        for (d = 0; (d < num_dimensions) && !break_search; d++) {
          ...
          if (dimension == 0) {
            if (!warning_produced)
              coco_warning(
                  "suite_biobj_get_new_instance(): remove filtering of "
                  "dimensions to get generally acceptable instances!");
            warning_produced = 1;
            continue;
          }

          break_search = suite_biobj_check_inst_consistency(
              dimension, function1, instance1, function2, instance2);
        }
      }
    }

    /* Finally, check all functions (f,f) with f in {f1, f2, f6, f8, f13, f14,
     * f15, f17, f20, f21}: */
    for (f1 = 0; (f1 < num_sel_bbob_functions) && !break_search; f1++) {
      ...
      for (d = 0; (d < num_dimensions) && !break_search; d++) {
        ...
        if (dimension == 0) {
          if (!warning_produced)
            coco_warning("suite_biobj_get_new_instance(): remove filtering of "
                         "dimensions to get generally acceptable instances!");
          warning_produced = 1;
          continue;
        }
    ...
    if (break_search) {
      ...
      coco_info("suite_biobj_get_new_instance(): Instance %lu created from "
                "instances %lu and %lu",
                (unsigned long)instance, (unsigned long)instance1,
                (unsigned long)instance2);
  ...
  if (!appropriate_instance_found) {
    coco_error("suite_biobj_get_new_instance(): Could not find suitable "
               "instance %lu in %lu tries",
 ...
 *
 * Creates the bi-objective problem by constructing it from two single-objective
 * problems. If the invoked instance number is not in suite_biobj_instances, the
 * function uses the following formula to construct a new appropriate instance:
 *   problem1_instance = 2 * biobj_instance + 1
 *   problem2_instance = problem1_instance + 1
 *
 * If needed, problem2_instance is increased (see also the explanation in
 * suite_biobj_get_new_instance).
 *
 * @param function Function
 * @param dimension Dimension
 * @param instance Instance
 * @param coco_get_problem_function The function that is used to access the
 ...
 * @param dimensions An array of dimensions to take into account when creating
 * new instances.
 * @param num_dimensions The number of dimensions to take into account when
 * creating new instances.
 * @return The problem that corresponds to the given parameters.
 */
static coco_problem_t *coco_get_biobj_problem(
  ...
  /* Selected functions from the bbob suite that are used to construct the
   * original bbob-biobj suite. */
   ...
   */
  const size_t all_bbob_functions[] = {1,  2,  3,  4,  5,  6,  7,  8,
  ...
  size_t i, j;
  const size_t num_existing_instances =
      sizeof(suite_biobj_instances) / sizeof(suite_biobj_instances[0]);
  ...
  if (function_idx < 55) {
    /* A "magic" formula to compute the BBOB function index from the
     * bi-objective function index */
    function1_idx =
        num_sel_bbob_functions -
        coco_double_to_size_t(
            floor(-0.5 + sqrt(0.25 + 2.0 * (double)(55 - function_idx - 1)))) -
        1;
    function2_idx = function_idx - (function1_idx * num_sel_bbob_functions) +
                    (function1_idx * (function1_idx + 1)) / 2;
  } else if (function_idx == 55) { /* There is not a simple "magic" formula for
                                      functions >= 55 */
    function1_idx = 0;
    function2_idx = 2;
    ...
    coco_error("coco_get_biobj_problem(): Invalid function index %i.",
               function_idx);
  }
  ...
  if (!instance_found) {
    ...
    if ((*new_inst_data) == NULL) {
      ...
      (*new_inst_data) = (suite_biobj_new_inst_t *)coco_allocate_memory(
          sizeof(**new_inst_data));

      /* Most often the actual number of new instances will be lower than
       * max_new_instances, because some of them are already in
       * suite_biobj_instances. However, in order to avoid iterating over
       * suite_biobj_new_inst_t, the allocation uses max_new_instances. */
      (*new_inst_data)->max_new_instances = num_new_instances;

      (*new_inst_data)->new_instances = (size_t **)coco_allocate_memory(
          (*new_inst_data)->max_new_instances * sizeof(size_t *));
      for (i = 0; i < (*new_inst_data)->max_new_instances; i++) {
        (*new_inst_data)->new_instances[i] =
            (size_t *)malloc(3 * sizeof(size_t));
        for (j = 0; j < 3; j++) {
    ...
    instance2 = suite_biobj_get_new_instance(
        (*new_inst_data), instance, instance1, all_bbob_functions,
        num_all_bbob_functions, sel_bbob_functions, num_sel_bbob_functions,
        dimensions, num_dimensions);
  }

  /* Construct the problem based on the function index and dimension */
  if (function_idx < 55) {
    problem1 = coco_get_problem_function(sel_bbob_functions[function1_idx],
                                         dimension, instance1);
    problem2 = coco_get_problem_function(sel_bbob_functions[function2_idx],
                                         dimension, instance2);
    ...
    problem1 = coco_get_problem_function(all_bbob_functions[function1_idx],
                                         dimension, instance1);
    problem2 = coco_get_problem_function(all_bbob_functions[function2_idx],
                                         dimension, instance2);
  ...
  /* Use the standard stacked problem_id as problem_name and construct a new
   * problem_id */
  coco_problem_set_name(problem, problem->problem_id);
  /* Attention! Any change to the problem id affects also archive processing! */
  coco_problem_set_id(problem, "bbob-biobj_f%02lu_i%02lu_d%02lu",
                      (unsigned long)function, (unsigned long)instance,
                      (unsigned long)dimension);

  /* Construct problem type */
  coco_problem_set_type(problem, "%s_%s", problem1->problem_type,
                        problem2->problem_type);
...
/**
 * @brief Returns the best known value for the hypervolume indicator matching
 * the given key if the key is found, and throws a coco_error otherwise.
 *
 * @note This function needs to be updated when a new biobjective suite is added
 * to COCO.
 */
static double suite_biobj_get_best_hyp_value(const char *suite_name,
  ...
  if ((strcmp(suite_name, "bbob-biobj") == 0) ||
    ...
    for (i = 0; i < count; i++) {
      sscanf(suite_biobj_mixint_best_values_hyp[i], "%s %lf", curr_key,
             &best_value);
      if (strcmp(curr_key, key) == 0) {
        coco_free_memory(curr_key);
        return best_value;
    ...
    coco_error("suite_biobj_get_best_hyp_value(): suite %s not supported",
               suite_name);
    return 0; /* Never reached */
  }

  coco_free_memory(curr_key);
  coco_warning("suite_biobj_get_best_hyp_value(): best value of %s could not "
               "be found; set to 1.0",
               key);
  return 1.0;
}
