 ...
 */
static coco_problem_t *f_attractive_sector_bbob_problem_allocate(
...
static coco_problem_t *f_attractive_sector_permblockdiag_bbob_problem_allocate(
    const size_t function, const size_t dimension, const size_t instance,
    const long rseed, const char *problem_id_template,
    const char *problem_name_template) {
  ...
  size_t *block_sizes1,
      *block_sizes2; /* each of R and Q might have its own parameter values */
  ...
  B1_copy = (const double *const *)
      B1; /* Wassim: silences the warning, not sure it prevents the modification
             of B at all levels*/
