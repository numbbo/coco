 ...
 * @brief Implementation of a bi-objective mixed-integer suite. The functions
 * are the same as those in the bbob-biobj-ext suite with 92 functions, but the
 * large-scale implementations of the functions are used instead of the original
 * ones for dimensions over 40.
 */
...
/**
 * @brief Sets the dimensions and default instances for the bbob-biobj-mixint
 * suite.
 */
static coco_suite_t *suite_biobj_mixint_initialize(void) {
  ...
  suite = coco_suite_allocate("bbob-biobj-mixint", 92, num_dimensions,
                              dimensions, "instances: 1-15");
...
/**
 * @brief Sets the instances associated with years for the bbob-biobj-mixint
 * suites.
 *
 * @note The instances of the bi-objective suites generally do not changes with
 * years.
 */
...
/**
 * @brief Creates and returns a mixed-integer bi-objective bbob problem without
 * needing the actual bbob-mixint suite.
 *
 * The problem is constructed by first finding the underlying single-objective
 * continuous problems, then discretizing the problems, then scaling them to
 * adjust their difficulty and finally stacking them to get a bi-objective
 ...
 */
static coco_problem_t *coco_get_biobj_mixint_problem(
  ...
  /* The cardinality of variables (0 = continuous variables should always come
   * last) */
  const size_t variable_cardinality[] = {2, 4, 8, 16, 0};
  size_t function1, function2;

  if (dimension % 5 != 0)
    coco_error("coco_get_biobj_mixint_problem(): dimension %lu not supported "
               "for suite_bbob_mixint",
               dimension);

  /* First, find the underlying single-objective continuous problems */
  problem_cont = coco_get_biobj_problem(
      function, dimension, instance, coco_get_problem_function, new_inst_data,
  ...
  assert(problem1);
  assert(problem2);
  /* Copy also the data of the underlying problems and set all pointers in such
   * a way that problem_cont can be safely freed */
  ...
  /* Set the ROI of the outer problem according to the given cardinality of
   * variables and the ROI of the inner problems to [-4, 4] for variables that
   * will be discretized */
  ...
  problem1_mixint =
      transform_vars_discretize(problem1, smallest_values_of_interest,
                                largest_values_of_interest, num_integer);
  problem2_mixint =
      transform_vars_discretize(problem2, smallest_values_of_interest,
                                largest_values_of_interest, num_integer);
  ...
  problem1_mixint = transform_obj_scale(
      problem1_mixint, suite_bbob_mixint_scaling_factors[function1 - 1]);
  problem2_mixint = transform_obj_scale(
      problem2_mixint, suite_bbob_mixint_scaling_factors[function2 - 1]);

  /* Fourth, combine the problems in a bi-objective mixed-integer problem */
  problem = coco_problem_stacked_allocate(problem1_mixint, problem2_mixint,
  ...
  coco_problem_set_id(problem, "bbob-biobj-mixint_f%03lu_i%02lu_d%03lu",
...
/**
 * @brief Returns the problem from the bbob-biobj-mixint suite that corresponds
 * to the given parameters.
 *
 * Uses large-scale bbob functions if dimension is equal or larger than the
 * hard-coded dim_large_scale value (50).
 *
...
suite_biobj_mixint_get_problem(coco_suite_t *suite, const size_t function_idx,
                               const size_t dimension_idx,
                               const size_t instance_idx) {

  coco_problem_t *problem = NULL;
  suite_biobj_new_inst_t *new_inst_data = (suite_biobj_new_inst_t *)suite->data;
  const size_t dim_large_scale =
      50; /* Switch to large-scale functions for dimensions over 50 */
  ...
  if (dimension < dim_large_scale)
    problem = coco_get_biobj_mixint_problem(
        function, dimension, instance, coco_get_bbob_problem, &new_inst_data,
        suite->number_of_instances, suite->dimensions,
        suite->number_of_dimensions);
  else
    problem = coco_get_biobj_mixint_problem(
        function, dimension, instance, coco_get_largescale_problem,
        &new_inst_data, suite->number_of_instances, suite->dimensions,
        suite->number_of_dimensions);
