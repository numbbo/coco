...
static const double fvalue_logged_for_infinite =
    3e21; /* value used for logging try */
static const double fvalue_logged_for_nan = 2e21;
/* static const double fvalue_logged_for_infeasible = 1e21;  only in first
 * evaluation */
static const double weight_constraints =
    1e0; /* factor used in logged indicator (f-f*)^+ + sum_i g_i^+ in front of
            the sum */

/*static const size_t bbob_nbpts_nbevals = 20; Wassim: tentative, are now
 * observer options with these default values*/
 ...
 */
static int bbob_logger_is_open =
...
static const char *data_format =
    "bbob-new2"; /* back to 5 columns, 5-th column writes single digit
                    constraint values */

/**
 * adds a formated line to a data file
 */
static void logger_bbob_write_data(
...
/*
 calling sequence:
 logger_bbob_open_dataFile(&(logger->fdata_file),
 logger->observer->output_folder, dataFile_path,
 ".dat");
 */

static void logger_bbob_open_dataFile(FILE **target_file, const char *path,
 ...
 */
static void
logger_bbob_openIndexFile(logger_bbob_data_t *logger, const char *folder_path,
                          const char *indexFile_prefix, const char *function_id,
                          const char *dataFile_path, const char *suite_name) {
  ...
  bbob_infoFile_firstInstance_char =
      coco_strdupf("%lu", (unsigned long)bbob_infoFile_firstInstance);
  ...
  strncat(file_name, bbob_infoFile_firstInstance_char,
    ...
    if ((tmp_file) && (bbob_current_dim == logger->number_of_variables) &&
      ...
      *target_file = fopen(file_path, "a+");
      ...
      if (bbob_current_dim != logger->number_of_variables) {
        int i, j;
        for (i = 0; i < bbob_number_of_dimensions &&
                    bbob_dimensions_in_current_infoFile[i] != 0 &&
                    bbob_dimensions_in_current_infoFile[i] !=
                        logger->number_of_variables;
             i++) {
          ; /* checks whether dimension already present in the current infoFile
             */
        }
        if (i < bbob_number_of_dimensions &&
            bbob_dimensions_in_current_infoFile[i] == 0) {
          /* new dimension seen for the first time */
          bbob_dimensions_in_current_infoFile[i] = logger->number_of_variables;
          newLine = 1;
        } else {
          if (i < bbob_number_of_dimensions) { /* dimension already present,
                                                  need to create a new file */
            newLine = 0;
            file_path[strlen(file_path) -
                      strlen(bbob_infoFile_firstInstance_char) - 7] =
            ...
            strncat(file_path, bbob_infoFile_firstInstance_char,
                    COCO_PATH_MAX - strlen(file_name) - 1);
            strncat(file_path, ".info", COCO_PATH_MAX - strlen(file_name) - 1);
          } else { /* we have all dimensions */
            newLine = 1;
          }
          for (j = 0; j < bbob_number_of_dimensions;
      ...
      *target_file = fopen(file_path, "a+"); /* in any case, we append */
      ...
      fprintf(*target_file,
              ...
              logger->observer->algorithm_name, coco_version, logger_name,
              data_format);
...
static void logger_bbob_initialize(logger_bbob_data_t *logger,
  ...
  char indexFile_prefix[10] =
      "bbobexp"; /* TODO (minor): make the prefix bbobexp a parameter that the
                    user can modify */
  ...
  logger_bbob_openIndexFile(logger, logger->observer->result_folder,
                            indexFile_prefix, tmpc_funId, dataFile_path,
                            coco_problem_get_suite(inner_problem)->suite_name);
  ...
  logger_bbob_open_dataFile(&(logger->tdata_file),
                            logger->observer->result_folder, dataFile_path,
                            ".tdat");
  fprintf(logger->tdata_file, bbob_file_header_str, logger->optimal_fvalue);

  logger_bbob_open_dataFile(&(logger->rdata_file),
                            logger->observer->result_folder, dataFile_path,
                            ".rdat");
 ...
 */
static void logger_bbob_evaluate(coco_problem_t *problem, const double *x,
                                 double *y) {
  size_t i;
  double y_logged, max_fvalue, sum_cons;
  double *cons = NULL;
  logger_bbob_data_t *logger =
      (logger_bbob_data_t *)coco_problem_transformed_get_data(problem);
  coco_problem_t *inner_problem =
...
static coco_problem_t *logger_bbob(coco_observer_t *observer,
  ...
  logger_data =
      (logger_bbob_data_t *)coco_allocate_memory(sizeof(*logger_data));
  logger_data->observer = observer;

  if (inner_problem->number_of_objectives != 1) {
    coco_warning("logger_bbob(): The bbob logger shouldn't be used to log a "
                 "problem with %d objectives",
                 inner_problem->number_of_objectives);
  }

  if (bbob_logger_is_open)
    coco_error("The current bbob_logger (observer) must be closed before a new "
               "one is opened");
  ...
  problem = coco_problem_transformed_allocate(
      inner_problem, logger_data, logger_bbob_free, observer->observer_name);
