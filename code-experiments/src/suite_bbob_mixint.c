 ...
 * @brief Implementation of a suite with mixed-integer bbob problems. The
 * functions are the same as those in the bbob suite with 24 functions, but the
 * large-scale implementations of the functions are used instead of the original
 * ones for dimensions over 40. Additionally, the functions are scaled as given
 * by suite_bbob_mixint_scaling_factors.
 */
...
/**
 * @brief Factors used to scale separate functions in order to achieve similar
 * difficulty.
 */
...
static coco_suite_t *suite_bbob_mixint_initialize(const char *suite_name) {
  ...
  suite = coco_suite_allocate(suite_name, 24, num_dimensions, dimensions,
                              "instances: 1-15");
...
/**
 * @brief Creates and returns a mixed-integer bbob problem without needing the
 * actual bbob-mixint suite.
 *
 ...
 */
static coco_problem_t *coco_get_bbob_mixint_problem(
  ...
  if (dimension % 5 != 0)
    coco_error("coco_get_bbob_mixint_problem(): dimension %lu not supported "
   ...
   * variables and the ROI of the inner problem to [-4, 4] for variables that
   * will be discretized */
  ...
  problem = transform_obj_scale(
      problem, suite_bbob_mixint_scaling_factors[function - 1]);

  coco_problem_set_id(problem, "bbob-mixint_f%03lu_i%02lu_d%02lu", function,
                      instance, dimension);
...
/**
 * @brief Returns the problem from the bbob-mixint suite that corresponds to the
 * given parameters.
 *
...
suite_bbob_mixint_get_problem(coco_suite_t *suite, const size_t function_idx,
  ...
  if (dimension < dim_large_scale)
    problem = coco_get_bbob_mixint_problem(function, dimension, instance,
                                           coco_get_bbob_problem);
  else
    problem = coco_get_bbob_mixint_problem(function, dimension, instance,
                                           coco_get_largescale_problem);
