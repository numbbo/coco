...
static size_t coco_rotation_matrix_block_size(size_t const dimension) {
  const double BLOCK_SIZE_RELATIVE =
      1; /* for block rotations, relative to dimension */
  const size_t MAX_BLOCK_SIZE_ABSOLUTE = 40; /* for block rotations */

  return coco_double_to_size_t(
      coco_double_min(BLOCK_SIZE_RELATIVE * (double)dimension,
                      (double)MAX_BLOCK_SIZE_ABSOLUTE));
}

/**
 * @brief
 * Allocate a ${n} by ${m} block matrix of nb_blocks block sizes block_sizes
 * structured as an array of pointers to double arrays. each row contains only
 * the block_sizes[i] possibly non-zero elements
 */
...
/**
 * @brief frees a block diagonal matrix (same as a matrix but in case of change,
 * easier to update separately from free_matrix)
 */
...
/**
 * @brief Compute a ${DIM}x${DIM} block-diagonal matrix based on ${seed} and
 * block_sizes and stores it in ${B}. B is a 2D vector with DIM lines and each
 * line has blocksize(line) elements (the zeros are not stored)
 */
static void coco_compute_blockrotation(double **B, long seed,
                                       COCO_UNUSED size_t n,
                                       size_t *block_sizes, size_t nb_blocks) {
  ...
  cumsum_prev_block_sizes =
      0; /* shift in rows to account for the previous blocks */
  for (idx_block = 0; idx_block < nb_blocks; idx_block++) {
    current_blocksize = block_sizes[idx_block];
    current_block =
        bbob2009_allocate_matrix(current_blocksize, current_blocksize);
    assert(current_blocksize <= 44);
    bbob2009_compute_rotation(current_block,
                              seed + (long)1000000 * (long)idx_block,
                              current_blocksize);
...
/**
 * @brief returns the list of block_sizes and sets nb_blocks to its correct
 * value
 */
static size_t *coco_get_block_sizes(size_t *nb_blocks, size_t dimension,
  ...
  if (strcmp(suite_name, "bbob-largescale") == 0) {
    /*block_size = coco_double_to_size_t(bbob2009_fmin((double)dimension / 4,
     * 100));*/ /*old value*/
    /*block_size = coco_double_to_size_t(bbob2009_fmin((double)dimension,
     * 40));*/
    ...
    *nb_blocks = dimension / block_size + ((dimension % block_size) > 0);
    ...
    block_sizes[*nb_blocks - 1] =
        dimension - (*nb_blocks - 1) * block_size; /*add rest*/
    return block_sizes;
  } else {
    return NULL;
  }
