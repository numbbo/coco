 ...
 * coco_suite_get_instances_by_year and coco_suite_get_problem_from_indices need
 * to be updated.
 *
 * @see <a href="index.html">Instructions</a> on how to write new test functions
...
/**
 * @brief Calls the function that sets the instanced by year for the given
 * suite.
 *
 * @note This function needs to be updated when a new suite is added to COCO.
 */
static const char *coco_suite_get_instances_by_year(const coco_suite_t *suite,
                                                    const int year) {
  ...
  if (strcmp(suite->suite_name, "bbob") == 0) {
    ...
    coco_error(
        "coco_suite_get_instances_by_year(): suite '%s' has no years defined",
        suite->suite_name);
    return NULL;
  }
...
/**
 * @brief Calls the function that returns the problem corresponding to the given
 * suite, function index, dimension index and instance index. If the indices
 * don't correspond to a problem because of suite filtering, it returns NULL.
 *
 * @note This function needs to be updated when a new suite is added to COCO.
 */
static coco_problem_t *coco_suite_get_problem_from_indices(
    coco_suite_t *suite, const size_t function_idx, const size_t dimension_idx,
  ...
  if (strcmp(suite->suite_name, "toy") == 0) {
    ...
    problem = suite_bbob_get_problem(suite, function_idx, dimension_idx,
                                     instance_idx);
  } else if (strcmp(suite->suite_name, "sbox-cost") == 0) {
    problem = suite_sbox_cost_get_problem(suite, function_idx, dimension_idx,
             ...
             (strcmp(suite->suite_name, "bbob-biobj-ext") == 0)) {
    problem = suite_biobj_get_problem(suite, function_idx, dimension_idx,
                                      instance_idx);
  } else if (strncmp(suite->suite_name, "bbob-constrained", 16) == 0) {
    problem = suite_cons_bbob_get_problem(suite, function_idx, dimension_idx,
                                          instance_idx);
  } else if (strcmp(suite->suite_name, "bbob-largescale") == 0) {
    problem = suite_largescale_get_problem(suite, function_idx, dimension_idx,
                                           instance_idx);
  } else if (strcmp(suite->suite_name, "bbob-mixint") == 0) {
    problem = suite_bbob_mixint_get_problem(suite, function_idx, dimension_idx,
                                            instance_idx);
  } else if (strcmp(suite->suite_name, "bbob-biobj-mixint") == 0) {
    problem = suite_biobj_mixint_get_problem(suite, function_idx, dimension_idx,
                                             instance_idx);
  } else {
...
static double coco_suite_get_best_indicator_value(const coco_suite_t *suite,
  ...
  if (strcmp(indicator_name, "hyp") == 0) {
    result =
        suite_biobj_get_best_hyp_value(suite->suite_name, problem->problem_id);
  } else {
    coco_error(
        "coco_suite_get_best_indicator_value(): indicator %s not supported",
        indicator_name);
...
/**
 * @note: While a suite can contain multiple problems with equal function,
 * dimension and instance, this function always returns the first problem in the
 * suite with the given function, dimension and instance values. If the given
 * values don't correspond to a problem, the function returns NULL.
 */
coco_problem_t *coco_suite_get_problem_by_function_dimension_instance(
    coco_suite_t *suite, const size_t function, const size_t dimension,
  ...
  return coco_suite_get_problem_from_indices(
      suite, (size_t)function_idx, (size_t)dimension_idx, (size_t)instance_idx);
}
...
static void coco_suite_set_instance(coco_suite_t *suite,
  ...
  suite->number_of_instances = coco_count_numbers(
      instance_numbers, COCO_MAX_INSTANCES, "suite instance numbers");
 ...
 *
 * Sets items[i - 1] to 0 for every i that cannot be found in indices (this
 * function performs the conversion from user-friendly indices starting from 1
 * to C-friendly indices starting from 0).
 */
static void coco_suite_filter_indices(size_t *items,
                                      const size_t number_of_items,
                                      const size_t *indices, const char *name) {
...
static void coco_suite_filter_dimensions(coco_suite_t *suite,
                                         const size_t *dimension_numbers) {

  size_t i, j;
  size_t count =
      coco_count_numbers(dimension_numbers, COCO_MAX_INSTANCES, "dimensions");
  int found;
 ...
 *
 * @return The function number in position function_idx in the suite. If the
...
size_t coco_suite_get_function_from_function_index(const coco_suite_t *suite,
  ...
  if (function_idx >= suite->number_of_functions) {
    coco_error("coco_suite_get_function_from_function_index(): function index "
               "exceeding the number of functions in the suite");
    return 0; /* Never reached*/
  }
...
/**
 * @param suite The given suite.
 * @param dimension_idx The index of the dimension in question (starting from
 * 0).
 *
 * @return The dimension number in position dimension_idx in the suite. If the
 * dimension has been filtered out through suite_options in the coco_suite
 * function, the result is 0.
 */
size_t
coco_suite_get_dimension_from_dimension_index(const coco_suite_t *suite,
                                              const size_t dimension_idx) {

  if (dimension_idx >= suite->number_of_dimensions) {
    coco_error("coco_suite_get_dimension_from_dimension_index(): dimensions "
               "index exceeding the number of dimensions in the suite");
    return 0; /* Never reached*/
  }
 ...
 *
 * @return The instance number in position instance_idx in the suite. If the
...
size_t coco_suite_get_instance_from_instance_index(const coco_suite_t *suite,
  ...
  if (instance_idx >= suite->number_of_instances) {
    coco_error("coco_suite_get_instance_from_instance_index(): instance index "
               "exceeding the number of instances in the suite");
    return 0; /* Never reached*/
  }
...
/**
 * Note that the problem_index depends on the number of instances a suite is
 * defined with.
 *
 * @param suite The given suite.
 * @param problem_index The index of the problem to be returned.
 *
 * @return The problem of the suite defined by problem_index (NULL if this
...
coco_problem_t *coco_suite_get_problem(coco_suite_t *suite,
  ...
  coco_suite_decode_problem_index(suite, problem_index, &function_idx,
                                  &dimension_idx, &instance_idx);

  return coco_suite_get_problem_from_indices(suite, function_idx, dimension_idx,
                                             instance_idx);
}

/**
 * The number of problems in the suite is computed as a product of the number of
 * instances, number of functions and number of dimensions and therefore doesn't
 * account for any filtering done through the suite_options parameter of the
 * coco_suite function.
 *
 * @param suite The given suite.
 *
...
/**
 * @brief Returns the instances read from either a "year: YEAR" or "instances:
 * NUMBERS" string.
 *
 * If both "year" and "instances" are given, the second is ignored (and a
 * warning is raised). See the coco_suite function for more information about
 * the required format.
 */
static size_t *coco_suite_get_instance_indices(const coco_suite_t *suite,
  ...
  if ((year_found > 0) && (instances_found > 0)) {
    if (year_found < instances_found) {
      parse_instances = 0;
      coco_warning("coco_suite_get_instance_indices(): 'instances' suite "
                   "option ignored because it follows 'year'");
    } else {
      parse_year = 0;
      coco_warning("coco_suite_get_instance_indices(): 'year' suite option "
                   "ignored because it follows 'instances'");
    }
  }
  ...
  if ((year_found >= 0) && (parse_year == 1)) {
    if (coco_options_read_int(suite_instance, "year", &(year)) != 0) {
      ...
      result = coco_string_parse_ranges(year_string, 1, 0, "instances",
                                        COCO_MAX_INSTANCES);
    } else {
      coco_warning("coco_suite_get_instance_indices(): problems parsing the "
                   "'year' suite_instance option, ignored");
    }
  }
  ...
  if ((instances_found >= 0) && (parse_instances == 1)) {
    if (coco_options_read_values(suite_instance, "instances", instances) > 0) {
      result = coco_string_parse_ranges(instances, 1, 0, "instances",
                                        COCO_MAX_INSTANCES);
    } else {
      coco_warning("coco_suite_get_instance_indices(): problems parsing the "
                   "'instance' suite_instance option, ignored");
    }
  }
...
/**
 * @brief Iterates through the items from the current_item_id position on in
 * search for the next positive item.
 *
 * If such an item is found, current_item_id points to this item and the method
 * returns 1. If such an item cannot be found, current_item_id points to the
 * first positive item and the method returns 0.
 */
static int coco_suite_is_next_item_found(const size_t *items,
                                         const size_t number_of_items,
  ...
  if ((*current_item_id) != (long)number_of_items - 1) {
    ...
    do {
      (*current_item_id)++;
    } while (((*current_item_id) < (long)number_of_items - 1) &&
             (items[*current_item_id] == 0));
  ...
  *current_item_id = -1;
  do {
    (*current_item_id)++;
           ...
           (items[*current_item_id] == 0));
  if (items[*current_item_id] == 0)
    coco_error("coco_suite_is_next_item_found(): the chosen suite has no valid "
               "(positive) items");
  return 0;
}

/**
 * @brief Iterates through the instances of the given suite from the
 * current_instance_idx position on in search for the next positive instance.
 *
 * If such an instance is found, current_instance_idx points to this instance
 * and the method returns 1. If such an instance cannot be found,
...
static int coco_suite_is_next_instance_found(coco_suite_t *suite) {
  ...
  return coco_suite_is_next_item_found(suite->instances,
                                       suite->number_of_instances,
                                       &suite->current_instance_idx);
}

/**
 * @brief Iterates through the functions of the given suite from the
 * current_function_idx position on in search for the next positive function.
 *
 * If such a function is found, current_function_idx points to this function and
 * the method returns 1. If such a function cannot be found,
 * current_function_idx points to the first positive function,
 * current_instance_idx points to the first positive instance and the method
 * returns 0.
 */
static int coco_suite_is_next_function_found(coco_suite_t *suite) {

  int result = coco_suite_is_next_item_found(suite->functions,
...
/**
 * @brief Iterates through the dimensions of the given suite from the
 * current_dimension_idx position on in search for the next positive dimension.
 *
 * If such a dimension is found, current_dimension_idx points to this dimension
 * and the method returns 1. If such a dimension cannot be found,
...
static int coco_suite_is_next_dimension_found(coco_suite_t *suite) {
  ...
  return coco_suite_is_next_item_found(suite->dimensions,
                                       suite->number_of_dimensions,
 ...
 * - "bbob" contains 24 <a
 * href="http://coco.lri.fr/downloads/download15.03/bbobdocfunctions.pdf">
 * single-objective functions</a> in 6 dimensions (2, 3, 5, 10, 20, 40)
 * - "bbob-biobj" contains 55 <a
 * href="http://numbbo.github.io/coco-doc/bbob-biobj/functions">bi-objective
 * functions</a> in 6 dimensions (2, 3, 5, 10, 20, 40)
 * - "bbob-biobj-ext" as an extension of "bbob-biobj" contains 92
 * <a href="http://numbbo.github.io/coco-doc/bbob-biobj/functions">bi-objective
 * functions</a> in 6 dimensions (2, 3, 5, 10, 20, 40)
 * - "bbob-largescale" contains 24 <a
 * href="http://coco.lri.fr/downloads/download15.03/bbobdocfunctions.pdf">
 * single-objective functions</a> in 6 large dimensions (40, 80, 160, 320, 640,
 * 1280)
 * - "bbob-constrained" contains 54 linearly-constrained problems, which are
 * combinations of 8 single objective functions with 6 different numbers of
 * active linear constraints (1, 2, 10, dimension/2, dimension-1, dimension+1),
 * in 6 dimensions (2, 3, 5, 10, 20, 40).
 * - "bbob-mixint" contains mixed-integer single-objective functions in 6
 * dimensions (2, 3, 5, 10, 20, 40)
 * - "bbob-biobj-mixint" contains 92 mixed-integer bi-objective functions in 6
 * dimensions (2, 3, 5, 10, 20, 40)
 * - "toy" contains 6 <a
 * href="http://coco.lri.fr/downloads/download15.03/bbobdocfunctions.pdf">
 ...
 * (default instances of a suite are those used in the last year and the suite
 * is not filtered by default).
 *
 * @param suite_name A string containing the name of the suite. Currently
 * supported suite names are "bbob", "bbob-biobj", "bbob-biobj-ext",
 * "bbob-largescale", "bbob-constrained", and "toy".
 * @param suite_instance A string used for defining the suite instances. Two
 * ways are supported:
 * - "year: YEAR", where YEAR is the year of the BBOB workshop, includes the
 * instances (to be) used in that year's workshop;
 * - "instances: VALUES", where VALUES are instance numbers from 1 on written as
 * a comma-separated list or a range m-n.
 * @param suite_options A string of pairs "key: value" used to filter the suite
 * (especially useful for parallelizing the experiments). Supported options:
 * - "dimensions: LIST", where LIST is the list of dimensions to keep in the
 * suite (range-style syntax is not allowed here),
 * - "dimension_indices: VALUES", where VALUES is a list or a range of dimension
 * indices (starting from 1) to keep in the suite, and
 * - "function_indices: VALUES", where VALUES is a list or a range of function
 * indices (starting from 1) to keep in the suite, and
 * - "instance_indices: VALUES", where VALUES is a list or a range of instance
 * indices (starting from 1) to keep in the suite.
 *
...
coco_suite_t *coco_suite(const char *suite_name, const char *suite_instance,
  ...
  const char *known_keys_o[] = {"dimensions", "dimension_indices",
                                "function_indices", "instance_indices"};
  ...
  if ((!suite_instance) || (strlen(suite_instance) == 0))
    instances =
        coco_suite_get_instance_indices(suite, suite->default_instances);
  else {
    ...
    if (given_option_keys) {
      ...
      if ((redundant_option_keys != NULL) &&
          (redundant_option_keys->count > 0)) {
        ...
        char *output_valid = coco_option_keys_get_output_string(
            known_option_keys, "Valid keys for suite instance are:\n");
  ...
  if ((suite_options) && (strlen(suite_options) > 0)) {
    ...
    if (coco_options_read_values(suite_options, "function_indices",
                                 option_string) > 0) {
      indices =
          coco_string_parse_ranges(option_string, 1, suite->number_of_functions,
                                   "function_indices", COCO_MAX_INSTANCES);
      if (indices != NULL) {
        coco_suite_filter_indices(suite->functions, suite->number_of_functions,
    ...
    option_string = coco_allocate_string(COCO_PATH_MAX + 1);
    if (coco_options_read_values(suite_options, "instance_indices",
                                 option_string) > 0) {
      indices =
          coco_string_parse_ranges(option_string, 1, suite->number_of_instances,
                                   "instance_indices", COCO_MAX_INSTANCES);
      if (indices != NULL) {
        coco_suite_filter_indices(suite->instances, suite->number_of_instances,
    ...
    if ((dim_found > 0) && (dim_idx_found > 0)) {
      if (dim_found < dim_idx_found) {
        parce_dim_idx = 0;
        coco_warning("coco_suite(): 'dimension_indices' suite option ignored "
                     "because it follows 'dimensions'");
      } else {
        parce_dim = 0;
        coco_warning("coco_suite(): 'dimensions' suite option ignored because "
                     "it follows 'dimension_indices'");
      }
    }

    option_string = coco_allocate_string(COCO_PATH_MAX + 1);
    if ((dim_idx_found >= 0) && (parce_dim_idx == 1) &&
        (coco_options_read_values(suite_options, "dimension_indices",
                                  option_string) > 0)) {
      indices = coco_string_parse_ranges(
          option_string, 1, suite->number_of_dimensions, "dimension_indices",
          COCO_MAX_INSTANCES);
      if (indices != NULL) {
        coco_suite_filter_indices(suite->dimensions,
                                  suite->number_of_dimensions, indices,
                                  "dimension_indices");
        coco_free_memory(indices);
      }
    }
    ...
    if ((dim_found >= 0) && (parce_dim == 1) &&
        (coco_options_read_values(suite_options, "dimensions", option_string) >
         0)) {
      ptr = option_string;
      /* Check for disallowed characters */
      while (*ptr != '\0') {
        if ((*ptr != ',') && !isdigit((unsigned char)*ptr)) {
          coco_warning("coco_suite(): 'dimensions' suite option ignored "
                       "because of disallowed characters");
          return NULL;
        } else
          ptr++;
      }
      dimensions = coco_string_parse_ranges(
          option_string, suite->dimensions[0],
          suite->dimensions[suite->number_of_dimensions - 1], "dimensions",
          COCO_MAX_INSTANCES);
      if (dimensions != NULL) {
    ...
    if (given_option_keys) {
      ...
      if ((redundant_option_keys != NULL) &&
        ...
        char *output_valid = coco_option_keys_get_output_string(
            known_option_keys, "Valid keys for suite options are:\n");
        coco_warning("%s%s", output_redundant, output_valid);
        coco_free_memory(output_redundant);
        coco_free_memory(output_valid);
      }
  ...
  if ((suite->number_of_dimensions < 1) || (suite->number_of_functions < 1) ||
      (suite->number_of_instances < 1)) {
    coco_error("coco_suite(): the suite does not contain at least one "
               "dimension, function and instance");
    return NULL;
  }

  /* Set the starting values of the current indices in such a way, that when the
   * instance_idx is incremented, this results in a valid problem */
 ...
 * information regarding the current place in the iteration. The returned
 * problem is wrapped with the observer. If the observer is NULL, the returned
 * problem is unobserved.
 *
 * @param suite The given suite.
 * @param observer The observer used to wrap the problem. If NULL, the problem
 * is returned unobserved.
 *
 * @returns The next problem of the suite or NULL if there is no next problem
 * left.
 */
coco_problem_t *coco_suite_get_next_problem(coco_suite_t *suite,
  ...
  /* Iterate through the suite by instances, then functions and lastly
   * dimensions in search for the next problem. Note that these functions set
   * the values of suite fields current_instance_idx, current_function_idx and
  ...
  problem = coco_suite_get_problem_from_indices(suite, function_idx,
                                                dimension_idx, instance_idx);
  if (observer != NULL)
  ...
  if (coco_log_level >= COCO_INFO) {
    if (((long)dimension_idx != previous_dimension_idx) ||
      ...
      coco_info_partial("COCO INFO: %s, d=%lu, running: f%02lu", time_string,
                        (unsigned long)suite->dimensions[dimension_idx],
                        (unsigned long)suite->functions[function_idx]);
      coco_free_memory(time_string);
    } else if ((long)function_idx != previous_function_idx) {
      /* A new function started */
      coco_info_partial("f%02lu",
                        (unsigned long)suite->functions[function_idx]);
    }
...
size_t coco_suite_encode_problem_index(const coco_suite_t *suite,
  ...
  return instance_idx + (function_idx * suite->number_of_instances) +
         (dimension_idx * suite->number_of_instances *
          suite->number_of_functions);
}
 ...
 * @param function_idx Pointer to the index of the function, which is set by
 * this function.
 * @param dimension_idx Pointer to the index of the dimension, which is set by
 * this function.
...
void coco_suite_decode_problem_index(const coco_suite_t *suite,
  ...
  *function_idx =
      (problem_index / suite->number_of_instances) % suite->number_of_functions;
  *dimension_idx =
      problem_index / (suite->number_of_instances * suite->number_of_functions);
}
