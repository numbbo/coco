 ...
 *
 * Logs the values of the implemented indicators and archives nondominated
 * solutions. Produces four kinds of files:
 * - The "info" files contain high-level information on the performed
 * experiment. One .info file is created for each problem group (and indicator
 * type) and contains information on all the problems in that problem group (and
 * indicator type).
 * - The "dat" files contain function evaluations, indicator values and target
 * hits for every target hit as well as for the last evaluation. One .dat file
 * is created for each problem function and dimension (and indicator type) and
 * contains information for all instances of that problem (and indicator type).
 * - The "tdat" files contain function evaluation and indicator values for every
 * predefined evaluation number as well as for the last evaluation. One .tdat
 * file is created for each problem function and dimension (and indicator type)
 * and contains information for all instances of that problem (and indicator
 * type).
 * - The "adat" files are archive files that contain function evaluations, 2
 * objectives and dim variables for every nondominated solution. Whether these
 * files are created, at what point in time the logger writes nondominated
 * solutions to the archive and whether the decision variables are output or not
 * depends on the values of log_nondom_mode and log_nondom_mode. See the
 * bi-objective observer constructor observer_biobj() for more information. One
 * .adat file is created for each problem function, dimension and instance.
 *
 * @note Whenever in this file a ROI is mentioned, it means the (normalized)
 * region of interest in the objective space. The non-normalized ROI is a
 * rectangle with the ideal and nadir points as its two opposite vertices, while
 * the normalized ROI is the square [0, 1]^2. If not specifically mentioned, the
 * normalized ROI is assumed.
 *
 * @note This logger can handle both the original bbob-biobj test suite with 55
 * and the extended bbob-biobj-ext test suite with 96 functions.
 */
 ...
 * */
const char *logger_biobj_indicators[LOGGER_BIOBJ_NUMBER_OF_INDICATORS] = {
    "hyp"};
 ...
 *
 * The hypervolume indicator measures the volume of the portion of the ROI in
 * the objective space that is dominated by the current Pareto front
 * approximation. Instead of logging the hypervolume indicator value, this
 * implementation logs the difference between the best know hypervolume
 * indicator (a value stored in best_value) and the hypervolume indicator of the
 * current Pareto front approximation (current_value). The current_value equals
 * 0 if no solution is located in the ROI. In order to be able to register the
 * performance of an optimizer even before the ROI is reached, an additional
 * value is computed when no solutions are located inside the ROI. This value is
 * stored in additional_penalty and equals the normalized distance to the ROI of
 * the solution closest to the ROI (additional_penalty is set to 0 as soon as a
 * solution reaches the ROI). The final value to be logged (overall_value) is
 * therefore computed in the following way:
 *
...
typedef struct {
  ...
  char *name; /**< @brief Name of the indicator used for identification and the
                 output. */

  FILE *dat_file;  /**< @brief File for logging indicator values at predefined
                      values. */
  FILE *tdat_file; /**< @brief File for logging indicator values at predefined
                      evaluations. */
  FILE *info_file; /**< @brief File for logging summary information on algorithm
                      performance. */

  int target_hit; /**< @brief Whether the target was hit in the latest
                     evaluation. */
  coco_observer_targets_t *targets;
  /**< @brief Triggers based on target values. */
  int evaluation_logged; /**< @brief Whether the whether the latest evaluation
                            was logged. */
  coco_observer_evaluations_t *evaluations;
  /**< @brief Triggers based on numbers of evaluations. */

  double best_value;         /**< @brief The best known indicator value for this
                                problem. */
  double current_value;      /**< @brief The current indicator value. */
  double additional_penalty; /**< @brief Additional penalty for solutions
                                outside the ROI. */
  double overall_value;  /**< @brief The overall value of the indicator tested
                            for target hits. */
  double previous_value; /**< @brief The previous overall value of the
                            indicator. */
 ...
 *
 * @note Some fields from the observers (coco_observer as well as
 * observer_biobj) need to be copied here because the observers can be deleted
 * before the logger is finalized and we need these fields for finalization.
 */
typedef struct {
  observer_biobj_log_nondom_e log_nondom_mode;
  /**< @brief Mode for archiving nondominated solutions. */
  FILE *adat_file; /**< @brief File for archiving nondominated solutions (all or
                      final). */
  ...
  size_t number_of_evaluations; /**< @brief The number of evaluations performed
                                   so far. */
  size_t number_of_variables;   /**< @brief Dimension of the problem. */
  size_t
      number_of_integer_variables; /**< @brief Number of integer variables. */
  size_t number_of_objectives; /**< @brief Number of objectives (clearly equal
                                  to 2). */
  size_t suite_dep_instance; /**< @brief Suite-dependent instance number of the
                                observed problem. */

  size_t previous_evaluations; /**< @brief The number of evaluations from the
                                  previous call to the logger. */

  avl_tree_t *archive_tree; /**< @brief The tree keeping currently non-dominated
                               solutions. */
  avl_tree_t *buffer_tree;  /**< @brief The tree with pointers to nondominated
                               solutions that haven't  been logged yet. */
...
/**
 * @brief The type for the node's item in the AVL tree as used by the
 * bi-objective logger.
 *
 * Contains information on the exact objective values (y) and their rounded
 * normalized values (normalized_y). The exact values are used for output, while
 * archive update and indicator computation use the normalized values.
 */
typedef struct {
  double *x; /**< @brief The decision values of this solution. */
  double *y; /**< @brief The values of objectives of this solution. */
  double *normalized_y; /**< @brief The values of normalized objectives of this
                           solution. */
  size_t evaluation_number; /**< @brief The evaluation number of when the
   ...
   */
  int within_ROI; /**< @brief Whether the solution is within the region of
                     interest (ROI). */

} logger_biobj_avl_item_t;

/**
 * @brief Creates and returns the information on the solution in the form of a
 * node's item in the AVL tree.
 */
static logger_biobj_avl_item_t *
logger_biobj_node_create(const coco_problem_t *problem, const double *x,
                         const double *y, const size_t evaluation_number,
                         const size_t dim, const size_t num_obj) {

  size_t i;

  /* Allocate memory to hold the data structure logger_biobj_node_t */
  logger_biobj_avl_item_t *item =
      (logger_biobj_avl_item_t *)coco_allocate_memory(sizeof(*item));
  ...
  item->normalized_y =
      mo_normalize(item->y, problem->best_value, problem->nadir_value, num_obj);
...
/**
 * @brief Defines the ordering of AVL tree nodes based on the value of the last
 * objective.
 *
 * @note This ordering is used by the archive_tree.
 */
static int
avl_tree_compare_by_last_objective(const logger_biobj_avl_item_t *item1,
                                   const logger_biobj_avl_item_t *item2,
                                   void *userdata) {
  (void)userdata; /* To silence the compiler */
  if (coco_double_almost_equal(item1->normalized_y[1], item2->normalized_y[1],
                               mo_precision))
    return 0;
...
/**
 * @brief Defines the ordering of AVL tree nodes based on the evaluation number
 * (the time when the nodes were created).
 *
...
/**
 * @brief Outputs the AVL tree to the given file. Returns the number of nodes in
 * the tree.
 */
static size_t
logger_biobj_tree_output(FILE *file, const avl_tree_t *tree, const size_t dim,
                         const size_t num_int_vars, const size_t num_obj,
                         const int log_vars, const int precision_x,
  ...
  if (tree->tail) {
    ...
    while (solution != NULL) {
      fprintf(file, "%lu\t",
              (unsigned long)((logger_biobj_avl_item_t *)solution->item)
                  ->evaluation_number);
      for (j = 0; j < num_obj; j++)
        fprintf(file, "%.*e\t", precision_f,
                ((logger_biobj_avl_item_t *)solution->item)->y[j]);
      if (log_vars) {
        for (i = 0; i < dim; i++)
          if ((i < num_int_vars) && (log_discrete_as_int))
            fprintf(file, "%d\t",
                    coco_double_to_int(
                        ((logger_biobj_avl_item_t *)solution->item)->x[i]));
          else
            fprintf(file, "%.*e\t", precision_x,
                    ((logger_biobj_avl_item_t *)solution->item)->x[i]);
      }
      fprintf(file, "\n");
 ...
 *
 * Checks for domination and updates the archive tree and the values of the
 * indicators if the given node is not weakly dominated by existing nodes in the
 * archive tree. This is where the main computation of indicator values takes
 * place.
 *
...
static int logger_biobj_tree_update(logger_biobj_data_t *logger,
  ...
  /* Find the first point that is not worse than the new point (NULL if such
  ...
  if (node == NULL) {
    ...
    dominance =
        mo_get_dominance(node_item->normalized_y,
                         ((logger_biobj_avl_item_t *)node->item)->normalized_y,
    ...
    if (dominance > -1) {
      ...
      if (dominance == 1) {
        ...
        if (logger->compute_indicators) {
          for (i = 0; i < LOGGER_BIOBJ_NUMBER_OF_INDICATORS; i++) {
            logger->indicators[i]->current_value -=
                ((logger_biobj_avl_item_t *)node->item)
                    ->indicator_contribution[i];
          }
        }
      ...
      /* The new point is dominated or equal to an existing one, nothing more to
       * do */
      trigger_update = 0;
    }
  }
  ...
  if (!trigger_update) {
    ...
    while (next_node != NULL) {
      /* Check the dominance relation between the new node and the next node.
       * There are only two possibilities: dominance = 0: the new node and the
       * next node are nondominated dominance = 1: the new node dominates the
       * next node */
      node = next_node;
      dominance = mo_get_dominance(
          node_item->normalized_y,
          ((logger_biobj_avl_item_t *)node->item)->normalized_y,
          logger->number_of_objectives);
      if (dominance == 1) {
        /* The new point dominates the next point, remove the next point */
    ...
    if (logger->compute_indicators) {
      if (node_item->within_ROI) {
        /* Compute indicator value for new node and update the indicator value
          ...
          if (next_item->within_ROI) {
            for (i = 0; i < LOGGER_BIOBJ_NUMBER_OF_INDICATORS; i++) {
              ...
              if (strcmp(logger->indicators[i]->name, "hyp") == 0) {
                next_item->indicator_contribution[i] =
                    (node_item->normalized_y[0] - next_item->normalized_y[0]) *
                    (1 - next_item->normalized_y[1]);
                assert(next_item->indicator_contribution[i] >= 0);
              } else {
                coco_error("logger_biobj_tree_update(): Indicator computation "
                           "not implemented yet for indicator %s",
            ...
            for (i = 0; i < LOGGER_BIOBJ_NUMBER_OF_INDICATORS; i++) {
              if (strcmp(logger->indicators[i]->name, "hyp") == 0) {
                node_item->indicator_contribution[i] =
                    (previous_item->normalized_y[0] -
                     node_item->normalized_y[0]) *
        ...
        if (previous_unavailable) {
          /* Previous item does not exist or is out of ROI, use reference point
           * instead */
          for (i = 0; i < LOGGER_BIOBJ_NUMBER_OF_INDICATORS; i++) {
            if (strcmp(logger->indicators[i]->name, "hyp") == 0) {
              node_item->indicator_contribution[i] =
                  (1 - node_item->normalized_y[0]) *
                  (1 - node_item->normalized_y[1]);
              assert(node_item->indicator_contribution[i] >= 0);
            } else {
              coco_error("logger_biobj_tree_update(): Indicator computation "
 ...
 */
static logger_biobj_indicator_t *logger_biobj_indicator(
    const logger_biobj_data_t *logger, const coco_observer_t *observer,
    const coco_problem_t *problem, const char *indicator_name) {
  ...
  indicator =
      (logger_biobj_indicator_t *)coco_allocate_memory(sizeof(*indicator));
  assert(observer);
  ...
  indicator->best_value = coco_suite_get_best_indicator_value(
      problem->suite, problem, indicator->name);
  ...
  indicator->targets = coco_observer_targets(observer->number_target_triggers,
                                             observer->target_precision);
  indicator->evaluations = coco_observer_evaluations(
      observer->base_evaluation_triggers, problem->number_of_variables);

  /* Prepare the info file */
  path_name = coco_allocate_string(COCO_PATH_MAX + 1);
  memcpy(path_name, observer->result_folder,
         strlen(observer->result_folder) + 1);
  ...
  if (!info_file_exists) {
    ...
    fprintf(indicator->info_file,
            "suite = '%s', algorithm = '%s', indicator = '%s', folder = '%s', "
            "coco_version = '%s'\n%% %s",
            problem->suite->suite_name, observer->algorithm_name,
            indicator_name, problem->problem_type, coco_version,
  ...
  if ((observer_data->previous_function != (long)problem->suite_dep_function) ||
      (observer_data->previous_dimension !=
       (long)problem->number_of_variables)) {
    fprintf(indicator->info_file, "\nfunction = %2lu, ",
            (unsigned long)problem->suite_dep_function);
    fprintf(indicator->info_file, "dim = %2lu, ",
  ...
  fprintf(indicator->dat_file, "%%\n%% index = %lu, name = %s\n",
          (unsigned long)problem->suite_dep_index, problem->problem_name);
  fprintf(indicator->dat_file, "%% instance = %lu, reference value = %.*e\n",
          (unsigned long)problem->suite_dep_instance, logger->precision_f,
          indicator->best_value);
  fprintf(indicator->dat_file,
...
static void logger_biobj_indicator_finalize(logger_biobj_indicator_t *indicator,
  ...
  fprintf(indicator->info_file, ", %lu:%lu|%.1e",
          (unsigned long)logger->suite_dep_instance,
 ...
 *
 * Outputs to the:
 * - dat file, if the archive was updated and a new target was reached for an
 * indicator;
 * - tdat file, if the number of evaluations matches one of the predefined
 * numbers.
 *
 * Note that a target is reached when
 * best_value - current_value + additional_penalty <= relative_target_value
 *
 * The relative_target_value is a target for indicator difference, not the
    ...
    for (i = 0; i < LOGGER_BIOBJ_NUMBER_OF_INDICATORS; i++) {
      ...
      if (update_performed) {
        ...
        if (strcmp(indicator->name, "hyp") == 0) {
          if (coco_double_almost_equal(indicator->current_value, 0,
                                       mo_precision)) {
            /* Update the additional penalty for hypervolume (the minimal
             * distance from the nondominated set to the ROI) */
            double new_distance = mo_get_distance_to_ROI(
                node_item->normalized_y, logger->number_of_objectives);
            indicator->additional_penalty =
                coco_double_min(indicator->additional_penalty, new_distance);
            assert(indicator->additional_penalty >= 0);
          } else {
            indicator->additional_penalty = 0;
          }
          indicator->overall_value = indicator->best_value -
                                     indicator->current_value +
                                     indicator->additional_penalty;
        } else {
          coco_error("logger_biobj_evaluate(): Indicator computation not "
                     "implemented yet for indicator %s",
                     indicator->name);
        }

        /* Check whether a target was hit */
        indicator->target_hit = coco_observer_targets_trigger(
            indicator->targets, indicator->overall_value);
      }
      ...
      if (logger->log_nondom_mode == LOG_NONDOM_READ) {
        /* Log to the tdat file the previous indicator value if any evaluation
         * number between the previous and this one matches one of the
         * predefined evaluation numbers. */
        for (j = logger->previous_evaluations + 1;
             j < logger->number_of_evaluations; j++) {
          indicator->evaluation_logged =
              coco_observer_evaluations_trigger(indicator->evaluations, j);
          if (indicator->evaluation_logged) {
            fprintf(indicator->tdat_file, "%lu\t%.*e\n", (unsigned long)j,
                    logger->precision_f, indicator->previous_value);
          }
        }
      }

      /* Log to the tdat file if the number of evaluations matches one of the
       * predefined numbers */
      indicator->evaluation_logged = coco_observer_evaluations_trigger(
          indicator->evaluations, logger->number_of_evaluations);
      if (indicator->evaluation_logged) {
        fprintf(indicator->tdat_file, "%lu\t%.*e\n",
                (unsigned long)logger->number_of_evaluations,
                logger->precision_f, indicator->overall_value);
...
/**
 * @brief Evaluates the function, increases the number of evaluations and
 * outputs information according to observer options.
 */
static void logger_biobj_evaluate(coco_problem_t *problem, const double *x,
                                  double *y) {
  ...
  node_item = logger_biobj_node_create(
      inner_problem, x, y, logger->number_of_evaluations,
      logger->number_of_variables, logger->number_of_objectives);

  /* Update the archive with the new solution, if it is not dominated by or
   * equal to existing solutions in the archive */
  update_performed = logger_biobj_tree_update(logger, node_item);

  /* If the archive was updated and you need to log all nondominated solutions,
   * output the new solution to nondom_file */
  if (update_performed && (logger->log_nondom_mode == LOG_NONDOM_ALL)) {
    logger_biobj_tree_output(
        logger->adat_file, logger->buffer_tree, logger->number_of_variables,
        logger->number_of_integer_variables, logger->number_of_objectives,
        logger->log_vars, logger->precision_x, logger->precision_f,
...
/**
 * Sets the number of evaluations, adds the objective vector to the archive and
 * outputs information according to observer options (but does not output the
 * archive).
 *
 * @note Vector y must point to a correctly sized allocated memory region and
 * the given evaluation number must be larger than the existing one.
 *
...
int coco_logger_biobj_feed_solution(coco_problem_t *problem,
                                    const size_t evaluation, const double *y) {
  ...
  if (logger->previous_evaluations >= evaluation)
    coco_error("coco_logger_biobj_reconstruct(): Evaluation %lu came before "
               "evaluation %lu. Note that "
               "the evaluations need to be always increasing.",
               logger->previous_evaluations, evaluation);
...
static void logger_biobj_finalize(logger_biobj_data_t *logger) {
  ...
  resorted_tree =
      avl_tree_construct((avl_compare_t)avl_tree_compare_by_eval_number, NULL);
  ...
  logger_biobj_tree_output(
      logger->adat_file, resorted_tree, logger->number_of_variables,
      logger->number_of_integer_variables, logger->number_of_objectives,
      logger->log_vars, logger->precision_x, logger->precision_f,
      logger->log_discrete_as_int);
...
static void logger_biobj_free(void *stuff) {
  ...
  if (((logger->log_nondom_mode == LOG_NONDOM_ALL) ||
       (logger->log_nondom_mode == LOG_NONDOM_FINAL)) &&
 ...
 *
 * Copies all observer field values that are needed after initialization into
 * logger field values for two reasons:
 * - If the observer is deleted before the suite, the observer is not available
 * anymore when the logger is finalized.
...
static coco_problem_t *logger_biobj(coco_observer_t *observer,
  ...
  if (inner_problem->number_of_objectives != 2) {
    coco_error("logger_biobj(): The bi-objective logger cannot log a problem "
               "with %d objective(s)",
  ...
  logger_data =
      (logger_biobj_data_t *)coco_allocate_memory(sizeof(*logger_data));
  ...
  observer_data = (observer_biobj_data_t *)observer->data;
  /* Copy values from the observes that you might need even if they do not exist
   * any more */
  ...
  if (((observer_data->log_vars_mode == LOG_VARS_LOW_DIM) &&
       (inner_problem->number_of_variables > 5)) ||
  ...
  if ((logger_data->log_nondom_mode == LOG_NONDOM_ALL) ||
    ...
    if (logger_data->log_nondom_mode == LOG_NONDOM_ALL)
      ...
      file_name =
          coco_strdupf("%s_nondom_final.adat", inner_problem->problem_id);
    coco_join_path(path_name, COCO_PATH_MAX, file_name, NULL);
    coco_free_memory(file_name);
    ...
    if (logger_data->log_vars) {
      fprintf(logger_data->adat_file,
              "%% function evaluation | %lu objectives | %lu variables\n",
  ...
  logger_data->archive_tree =
      avl_tree_construct((avl_compare_t)avl_tree_compare_by_last_objective,
                         (avl_free_t)logger_biobj_node_free);
  logger_data->buffer_tree =
      avl_tree_construct((avl_compare_t)avl_tree_compare_by_eval_number, NULL);

  /* Initialize the indicators */
  if (logger_data->compute_indicators) {
    for (i = 0; i < LOGGER_BIOBJ_NUMBER_OF_INDICATORS; i++)
      logger_data->indicators[i] = logger_biobj_indicator(
          logger_data, observer, inner_problem, logger_biobj_indicators[i]);

    observer_data->previous_function = (long)inner_problem->suite_dep_function;
    observer_data->previous_dimension =
        (long)inner_problem->number_of_variables;
  }

  problem = coco_problem_transformed_allocate(
      inner_problem, logger_data, logger_biobj_free, observer->observer_name);
