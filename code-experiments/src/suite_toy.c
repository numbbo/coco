/**
 * @file suite_toy.c
 * @brief Implementation of a toy suite containing 6 noiseless "basic"
 * single-objective functions in 5 dimensions.
 */
...
static coco_suite_t *suite_toy_initialize(void) {
  ...
  suite =
      coco_suite_allocate("toy", 6, num_dimensions, dimensions, "instances: 1");

  return suite;
}

/**
 * @brief Returns the problem from the toy suite that corresponds to the given
 * parameters.
 *
...
static coco_problem_t *suite_toy_get_problem(coco_suite_t *suite,
  ...
  if (function == 1) {
    ...
    coco_error(
        "suite_toy_get_problem(): function %lu does not exist in this suite",
