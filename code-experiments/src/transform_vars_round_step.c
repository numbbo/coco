 ...
 * @brief Implementation of rounding the variables for the step-ellipsoid
 * function
 ...
 */
typedef struct {
  double zhat_1; /**< @brief contains the value of \hat{z}_1 that is used to
                    compute the fintess */
...
static void transform_vars_round_step_evaluate(coco_problem_t *problem,
  ...
  data = (transform_vars_round_step_data_t *)coco_problem_transformed_get_data(
      problem);
  ...
  ((f_step_ellipsoid_versatile_data_t *)problem->versatile_data)->zhat_1 =
      ...
      (double)inner_problem
          ->number_of_variables; /* TODO: Discuss: consider not pre-imptively
                                    multiplying by dim to not change the outcome
                                    of the max in the core function even though
                                    we might want to keep it as it is since
                                    otherwise, the sum part of the max may take
                                    over as dim increases */
...
static void transform_vars_round_step_free(void *thing) {
  transform_vars_round_step_data_t *data =
      (transform_vars_round_step_data_t *)thing;
...
static coco_problem_t *transform_vars_round_step(coco_problem_t *inner_problem,
  ...
  data =
      (transform_vars_round_step_data_t *)coco_allocate_memory(sizeof(*data));
  data->rounded_x =
      coco_allocate_vector(inner_problem->number_of_variables + 1);
  data->alpha = alpha;

  problem = coco_problem_transformed_allocate(inner_problem, data,
                                              transform_vars_round_step_free,
  ...
  for (i = 0; i < problem->number_of_variables; i++) {
    if (fabs(problem->best_parameter[i]) > 0.5) {
      problem->best_parameter[i] =
          coco_double_round(problem->best_parameter[i]);
    } else {
      problem->best_parameter[i] =
          coco_double_round(data->alpha * problem->best_parameter[i]) /
          data->alpha;
    }
  }
  return problem;
