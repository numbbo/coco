 ...
 * @brief Implementation of the ominous 's_i scaling' of the BBOB
 * Bueche-Rastrigin problem.
 */
 ...
 */
static void transform_vars_brs_evaluate(coco_problem_t *problem,
  ...
  data =
      (transform_vars_brs_data_t *)coco_problem_transformed_get_data(problem);
...
static coco_problem_t *transform_vars_brs(coco_problem_t *inner_problem) {
  ...
  problem = coco_problem_transformed_allocate(
      inner_problem, data, transform_vars_brs_free, "transform_vars_brs");
  problem->evaluate_function = transform_vars_brs_evaluate;

  if (coco_problem_best_parameter_not_zero(inner_problem)) {
    coco_warning(
        "transform_vars_brs(): 'best_parameter' not updated, set to NAN");
