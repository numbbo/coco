...
/**
 * @brief Inverse of oscillating non-linear transformation tosz_uv obtained with
 * brentq.
 */
 ...
 */
static void tosz(transform_vars_oscillate_data_t *data, const double *x,
...
static void transform_vars_oscillate_evaluate_function(coco_problem_t *problem,
  ...
  data = (transform_vars_oscillate_data_t *)coco_problem_transformed_get_data(
      problem);
 ...
 */
static void
transform_vars_oscillate_evaluate_constraint(coco_problem_t *problem,
...
static void transform_vars_oscillate_free(void *thing) {
  transform_vars_oscillate_data_t *data =
      (transform_vars_oscillate_data_t *)thing;
...
static coco_problem_t *transform_vars_oscillate(coco_problem_t *inner_problem) {
  ...
  problem = coco_problem_transformed_allocate(inner_problem, data,
                                              transform_vars_oscillate_free,
                                              "transform_vars_oscillate");
...
/**
 * @brief Applies the inverse of the oscillating transformation tasy(tosz(.)) to
 ...
 */
static void transform_inv_initial_oscillate(coco_problem_t *problem,
                                            const double *xopt) {
  size_t i;
  size_t j;
  int is_in_bounds;
  ...
  while (1) {
    ...
    for (i = 0; i < problem->number_of_variables; ++i) {
      di = tosz_uv_inv(problem->initial_solution[i] *
