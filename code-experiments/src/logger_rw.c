 ...
 *
 * Can be used to log all (or just those that are better than the preceding)
 * solutions with information about objectives, decision variables (optional)
 * and constraints (optional). See observer_rw() for more information on the
 * options. Produces one "txt" file for each problem function, dimension and
 * instance.
 *
 * @note This logger can be used with single- and multi-objective problems, but
 * in the multi-objective case, all solutions are always logged.
 */
 ...
 *
 * @note Some fields from the observers (coco_observer as well as observer_rw)
 * need to be copied here because the observers can be deleted before the logger
 * is finalized and we need these fields for finalization.
 */
typedef struct {
...
/**
 * @brief Evaluates the function and constraints and outputs the information
 * according to the observer options.
 */
static void logger_rw_evaluate(coco_problem_t *problem, const double *x,
                               double *y) {
  ...
  if (log_this_time) {
    ...
    if (logger->log_cons) {
      for (i = 0; i < problem->number_of_constraints; i++)
        fprintf(logger->out_file, "%.*e\t", logger->precision_g,
                constraints[i]);
    }
    /* Log time in seconds */
    if (logger->log_time)
...
static coco_problem_t *logger_rw(coco_observer_t *observer,
  ...
  fprintf(logger_data->out_file,
          "\n%% suite = '%s', problem_id = '%s', problem_name = '%s', "
          "coco_version = '%s'\n",
  ...
  problem = coco_problem_transformed_allocate(
      inner_problem, logger_data, logger_rw_free, observer->observer_name);
