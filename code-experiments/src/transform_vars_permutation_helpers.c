...
static double *perm_random_data; /* global variable used to generate the random
                                    permutations */

/**
 * @brief Comparison function used for sorting. In our case, it serves as a
 * random permutation generator
 */
static int f_compare_doubles_for_random_permutation(const void *a,
                                                    const void *b) {
  double temp = perm_random_data[*(const size_t *)a] -
                perm_random_data[*(const size_t *)b];
  if (temp > 0)
    return 1;
  else if (temp < 0)
    return -1;
 ...
 */
static void coco_compute_permutation_from_sequence(size_t *P, double *seq,
                                                   size_t length) {
  size_t i;
...
/**
 * @brief returns a uniformly distributed integer between lower_bound and
 * upper_bound using seed without using coco_random_new. Move to coco_utilities?
 */
static long coco_random_unif_integer(long lower_bound, long upper_bound,
                                     long seed) {
  long range, rand_int;
  double *tmp_uniform;
...
/**
 * @brief generates a random permutation resulting from nb_swaps truncated
 * uniform swaps of range swap_range missing parameters: dynamic_not_static
 * pool, seems empirically irrelevant for now so dynamic is implemented (simple
 * since no need for tracking indices if swap_range is 0, a random uniform
 * permutation is generated
 */
static void coco_compute_truncated_uniform_swap_permutation(size_t *P,
                                                            long seed, size_t n,
                                                            size_t nb_swaps,
                                                            size_t swap_range) {
  size_t i, idx_swap;
  ...
  if (swap_range > 0) {
    /*sort the random data in perm_random_data and arrange idx_order
     * accordingly*/
    /*did not use coco_compute_random_permutation to only use the seed once*/
    qsort(idx_order, n, sizeof(size_t),
          f_compare_doubles_for_random_permutation);
    for (idx_swap = 0; idx_swap < nb_swaps; idx_swap++) {
      ...
      second_swap_var = (size_t)coco_random_unif_integer(
          (long)lower_bound, (long)upper_bound - 1,
 ...
 */
static size_t *coco_duplicate_size_t_vector(const size_t *src,
...
/**
 * @brief return the number of swaps corresponding to the problem in the given
 * suite
 */
