 ...
 */
static double f_schwefel_generalized_raw(const double *x,
  ...
  result = 0.01 * (penalty + Schwefel_constant * 100. -
                   sum / (double)number_of_variables);
 ...
 */
static void f_schwefel_generalized_evaluate(coco_problem_t *problem,
...
f_schwefel_generalized_allocate(const size_t number_of_variables) {

  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "Schwefel function", f_schwefel_generalized_evaluate, NULL,
  ...
  f_schwefel_generalized_evaluate(problem, problem->best_parameter,
                                  problem->best_value);
  return problem;
}
 ...
 */
static coco_problem_t *f_schwefel_generalized_bbob_problem_allocate(
  ...
  size_t i;
  /*const double Schwefel_constant = 4.189828872724339;*/ /* Wassim: now inside
                                                             the raw function*/
  ...
  /*problem = transform_obj_norm_by_dim(problem);*/ /* Wassim: there is already
                                                       a normalization by
                                                       dimension*/
  /*problem = transform_obj_shift(problem, Schwefel_constant);*/ /* Wassim: now
                                                                    in the raw
                                                                    definition
                                                                  */
