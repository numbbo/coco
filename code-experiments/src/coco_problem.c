 ...
 * @param y The objective vector that is the result of the evaluation (in
 * single-objective problems only the first vector item is being set).
 */
void coco_evaluate_function(coco_problem_t *problem, const double *x,
                            double *y) {
  ...
  /* Set objective vector to INFINITY if the decision vector contains any
   * INFINITY values */
  ...
  problem->evaluations++; /* each derived class has its own counter, only the
                             most outer will be visible */
 ...
 */
void coco_evaluate_constraint(coco_problem_t *problem, const double *x,
                              double *y) {
  /* implements a safer version of problem->evaluate(problem, x, y) */
  size_t i, j;
  assert(problem != NULL);
  if (problem->evaluate_constraint == NULL) {
    coco_error("coco_evaluate_constraint(): No constraint function implemented "
               "for problem %s",
               problem->problem_id);
  }

  /* Set constraints vector to INFINITY if the decision vector contains any
   * INFINITY values */
  for (i = 0; i < coco_problem_get_dimension(problem); i++) {
    if (coco_is_inf(x[i])) {
  ...
  /* Set constraints vector to NAN if the decision vector contains any NAN
   * values */
  if (coco_vector_contains_nan(x, coco_problem_get_dimension(problem))) {
 ...
 */
static void bbob_evaluate_gradient(coco_problem_t *problem, const double *x,
                                   double *y) {
  /* implements a safer version of problem->evaluate_gradient(problem, x, y) */
  assert(problem != NULL);
  if (problem->evaluate_gradient == NULL) {
    coco_error("bbob_evaluate_gradient(): No gradient function implemented for "
               "problem %s",
               problem->problem_id);
  }
...
/**
 * Evaluates and logs the given solution (as the coco_evaluate_function), but
 does not return the evaluated
 * value.
 *
...
void coco_recommend_solution(coco_problem_t *problem, const double *x) {
  ...
  if (problem->recommend_solution == NULL) {
    coco_error("coco_recommend_solutions(): No recommend solution function "
               "implemented for problem %s",
               problem->problem_id);
  }
...
/**
 * @brief Allocates a new coco_problem_t for the given number of variables,
 * number of objectives and number of constraints.
 */
static coco_problem_t *
coco_problem_allocate(const size_t number_of_variables,
  ...
  problem->number_of_integer_variables =
      0; /* No integer variables by default */
  ...
  problem->final_target_delta[0] =
      1e-8; /* in case to be modified by the benchmark */
  problem->best_observed_fvalue[0] = DBL_MAX;
  problem->best_observed_evaluation[0] = 0;
  problem->suite = NULL; /* To be initialized in the
                            coco_suite_get_problem_from_indices() function */
  problem->suite_dep_index = 0;
  problem->suite_dep_function = 0;
  problem->suite_dep_instance = 0;
  problem->data = NULL;
  problem->versatile_data = NULL; /* Wassim: added to be able to pass data from
                                     one transformation to another*/
  return problem;
}

/**
 * @brief Creates a duplicate of the 'other' problem for all fields except for
 * data, which points to NULL.
 */
static coco_problem_t *coco_problem_duplicate(const coco_problem_t *other) {
  size_t i;
  coco_problem_t *problem;
  problem = coco_problem_allocate(other->number_of_variables,
  ...
  problem->versatile_data =
      other->versatile_data; /* Wassim: make the pointers the same*/
  ...
  if (other->initial_solution)
    problem->initial_solution = coco_duplicate_vector(
        other->initial_solution, other->number_of_variables);
...
/**
 * @brief Allocates a problem using scalar values for
 * smallest_value_of_interest, largest_value_of_interest and best_parameter.
 * Assumes all variables are continuous.
 */
static coco_problem_t *coco_problem_allocate_from_scalars(
    const char *problem_name, coco_evaluate_function_t evaluate_function,
    coco_problem_free_function_t problem_free_function,
    const size_t number_of_variables, const double smallest_value_of_interest,
...
/**
 * @brief Checks whether the given string is in the right format to be a
 * problem_id.
 *
 ...
 *
 * Takes care of memory (de-)allocation and verifies that the problem_id is in
 * the correct format.
 */
...
/**
 * @brief Returns 1 if the best parameter is not (close to) zero and 0
 * otherwise.
 */
static int coco_problem_best_parameter_not_zero(const coco_problem_t *problem) {
  ...
  if (coco_vector_contains_nan(problem->best_parameter,
                               problem->number_of_variables))
    return 1;

  while (i < problem->number_of_variables && best_is_zero) {
    best_is_zero =
        coco_double_almost_equal(problem->best_parameter[i], 0, 1e-9);
    i++;
  }
...
int coco_problem_final_target_hit(const coco_problem_t *problem) {
  ...
  return problem->best_observed_fvalue[0] <=
                 problem->best_value[0] + problem->final_target_delta[0]
             ? 1
             : 0;
}
/**
...
/**
 * @note Do not modify the returned string! If you free the problem, the
 * returned pointer becomes invalid. When in doubt, use coco_strdup() on the
 * returned value.
 */
...
/**
 * The ID is guaranteed to contain only characters in the set [a-z0-9_-]. It
 * should therefore be safe to use it to construct filenames or other
 * identifiers.
 *
...
const double *
coco_problem_get_smallest_values_of_interest(const coco_problem_t *problem) {
  assert(problem != NULL);
...
const double *
coco_problem_get_largest_values_of_interest(const coco_problem_t *problem) {
  assert(problem != NULL);
...
size_t
coco_problem_get_number_of_integer_variables(const coco_problem_t *problem) {
  assert(problem != NULL);
  return problem->number_of_integer_variables;
}

const double *
coco_problem_get_largest_fvalues_of_interest(const coco_problem_t *problem) {
  assert(problem != NULL);
  if (problem->number_of_objectives == 1)
    coco_error("coco_problem_get_largest_fvalues_of_interest(): f-values of "
               "interest undefined for single-objective problems");
  if (problem->nadir_value == NULL)
    coco_error("coco_problem_get_largest_fvalues_of_interest(): f-values of "
               "interest undefined");
 ...
 * initial solution. Takes care of rounding the solution in case of integer
 * variables.
 *
 * @param problem The given COCO problem.
 * @param initial_solution The pointer to the initial solution being set by this
 * method.
 */
  ...
  if (problem->initial_solution != NULL) {
    ...
    for (i = 0; i < problem->number_of_variables; ++i)
      initial_solution[i] = problem->smallest_values_of_interest[i] +
                            0.5 * (problem->largest_values_of_interest[i] -
                                   problem->smallest_values_of_interest[i]);
...
static void coco_problem_set_suite(coco_problem_t *problem,
                                   coco_suite_t *suite) {
  assert(problem != NULL);
  problem->suite = suite;
}
...
static size_t
coco_problem_get_suite_dep_function(const coco_problem_t *problem) {
  assert(problem != NULL);
...
static size_t
coco_problem_get_suite_dep_instance(const coco_problem_t *problem) {
  assert(problem != NULL);
 ...
 */
static coco_problem_t *
coco_problem_transformed_get_inner_problem(const coco_problem_t *problem) {
  assert(problem != NULL);
  assert(problem->data != NULL);
  assert(((coco_problem_transformed_data_t *)problem->data)->inner_problem !=
         NULL);
 ...
 */
static void coco_problem_transformed_evaluate_function(coco_problem_t *problem,
                                                       const double *x,
                                                       double *y) {
 ...
 */
static void
coco_problem_transformed_evaluate_constraint(coco_problem_t *problem,
...
static void bbob_problem_transformed_evaluate_gradient(coco_problem_t *problem,
                                                       const double *x,
                                                       double *y) {
  coco_problem_transformed_data_t *data;
  assert(problem != NULL);
 ...
 */
static void coco_problem_transformed_recommend_solution(coco_problem_t *problem,
                                                        const double *x) {
  coco_problem_transformed_data_t *data;
  assert(problem != NULL);
  assert(problem->data != NULL);
...
/**
 * @brief Frees only the data of the transformed problem leaving the inner
 * problem intact.
 *
 * @note If there is no other pointer to the inner problem, access to it will be
 * lost.
 */
static void coco_problem_transformed_free_data(coco_problem_t *problem) {
  ...
  /* Let the generic free problem code deal with the rest of the fields. For
   * this we clear the free_problem function pointer and recall the generic
   * function. */
  problem->problem_free_function = NULL;
 ...
 *
 * By default all methods will dispatch to the inner_problem. A prefix is
 * prepended to the problem name in order to reflect the transformation
 * somewhere.
 */
static coco_problem_t *coco_problem_transformed_allocate(
    coco_problem_t *inner_problem, void *user_data,
    coco_data_free_function_t data_free_function, const char *name_prefix) {
  coco_problem_transformed_data_t *problem;
  coco_problem_t *inner_copy;
  char *old_name = coco_strdup(inner_problem->problem_name);

  problem =
      (coco_problem_transformed_data_t *)coco_allocate_memory(sizeof(*problem));
  ...
  inner_copy->evaluate_constraint =
      coco_problem_transformed_evaluate_constraint;
 ...
 */
static void coco_problem_stacked_evaluate_function(coco_problem_t *problem,
                                                   const double *x, double *y) {
  coco_problem_stacked_data_t *data =
      (coco_problem_stacked_data_t *)problem->data;

  const size_t number_of_objectives_problem1 =
      coco_problem_get_number_of_objectives(data->problem1);
  const size_t number_of_objectives_problem2 =
  ...
  if (number_of_objectives_problem2 > 0)
    coco_evaluate_function(data->problem2, x,
                           &y[number_of_objectives_problem1]);
...
/**
 * @brief Calls the coco_evaluate_constraint function on the underlying
 * problems.
 */
static void coco_problem_stacked_evaluate_constraint(coco_problem_t *problem,
                                                     const double *x,
                                                     double *y) {
  coco_problem_stacked_data_t *data =
      (coco_problem_stacked_data_t *)problem->data;

  const size_t number_of_constraints_problem1 =
      coco_problem_get_number_of_constraints(data->problem1);
  const size_t number_of_constraints_problem2 =
  ...
  if (number_of_constraints_problem2 > 0)
    coco_evaluate_constraint(data->problem2, x,
                             &y[number_of_constraints_problem1]);
}
 ...
 *
 * This is particularly useful for generating multi-objective problems, e.g. a
 * bi-objective problem from two single-objective problems. The stacked problem
 * must behave like a normal COCO problem accepting the same input.
 *
 * @note Regions of interest in the decision space must either agree or at least
 * one of them must be NULL.
 * @note Best parameter becomes somewhat meaningless, but the nadir value make
 * sense now.
 */
static coco_problem_t *
coco_problem_stacked_allocate(coco_problem_t *problem1,
  ...
  assert(problem1);
  assert(problem2);
  assert(coco_problem_get_dimension(problem1) ==
         coco_problem_get_dimension(problem2));
  ...
  problem = coco_problem_allocate(number_of_variables, number_of_objectives,
  ...
  assert(problem1->number_of_integer_variables ==
         problem2->number_of_integer_variables);
  ...
  if (number_of_constraints > 0) {
    ...
    if (coco_problem_get_number_of_constraints(problem2) > 0 &&
        problem2->initial_solution)
      problem->initial_solution = coco_duplicate_vector(
          problem2->initial_solution, number_of_variables);
    ...
    coco_evaluate_function(problem1, problem2->best_parameter,
                           &problem->nadir_value[0]);
    coco_evaluate_function(problem2, problem1->best_parameter,
                           &problem->nadir_value[1]);
  }
