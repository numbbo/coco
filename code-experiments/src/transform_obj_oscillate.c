...
static void transform_obj_oscillate_evaluate(coco_problem_t *problem,
  ...
  for (i = 0; i < problem->number_of_objectives; i++) {
    if (y[i] != 0) {
      ...
      if (y[i] > 0) {
        y[i] =
            pow(exp(log_y + 0.49 * (sin(log_y) + sin(0.79 * log_y))), factor);
      } else {
        y[i] = -pow(exp(log_y + 0.49 * (sin(0.55 * log_y) + sin(0.31 * log_y))),
                    factor);
      }
    }
  }
