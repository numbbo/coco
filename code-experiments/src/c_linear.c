...
static void c_sum_variables_evaluate(coco_problem_t *self, const double *x,
                                     double *y);

static void c_linear_single_evaluate(coco_problem_t *self, const double *x,
                                     double *y);
...
c_sum_variables_allocate(const size_t number_of_variables);

static coco_problem_t *c_linear_transform(coco_problem_t *inner_problem,
                                          const double *gradient,
                                          const double x_shift_factor);

static coco_problem_t *c_linear_single_cons_bbob_problem_allocate(
    const size_t function, const size_t dimension, const size_t instance,
    const size_t constraint_number, const double factor1,
    const char *problem_id_template, const char *problem_name_template,
    double *gradient, double x_shift_factor, const double *feasible_direction);
...
static void c_sum_variables_evaluate(coco_problem_t *self, const double *x,
  ...
  for (i = 0; i < self->number_of_variables; ++i)
    /* prevent that the optimal solution is infeasible due to loss of precision
     * when shifted */
    if (x[i] > 1e-11 || x[i] < 0)
      y[0] += x[i];
}
 ...
 */
static coco_problem_t *
c_guarantee_feasible_point(coco_problem_t *problem,
  ...
  problem->initial_solution =
      coco_duplicate_vector(feasible_direction, problem->number_of_variables);
...
c_sum_variables_allocate(const size_t number_of_variables) {
  ...
  coco_problem_set_id(problem, "%s_d%02lu", "linearconstraint",
                      number_of_variables);
...
static coco_problem_t *c_linear_transform(coco_problem_t *inner_problem,
  ...
  coco_problem_t *self;
  double gradient_norm =
      coco_vector_norm(gradient, inner_problem->number_of_variables);
  ...
  data->gradient =
      coco_duplicate_vector(gradient, inner_problem->number_of_variables);
  ...
  self = coco_problem_transformed_allocate(inner_problem, data,
                                           c_linear_gradient_free,
                                           "gradient_linear_constraint");
...
static coco_problem_t *c_linear_single_cons_bbob_problem_allocate(
    ...
    const size_t constraint_number, const double factor1,
    const char *problem_id_template, const char *problem_name_template,
    double *gradient, const double x_shift_factor,
    const double *feasible_direction) {
  ...
  seed_cons_i = (long)(function + 10000 * instance + 50000 * constraint_number);
  ...
  if (gradient) {
    ...
    coco_vector_scale(gradient, dimension, factor1 * factor2,
    ...
    for (i = 0; i < dimension; ++i)
      gradient_linear_constraint[i] = factor1 *
                                      coco_random_normal(random_generator) *
                                      factor2 / sqrt((double)dimension);

    problem =
        c_linear_transform(problem, gradient_linear_constraint, x_shift_factor);
...
static void con_update_linear_combination(double *linear_combination,
  ...
  if (data->gradient == NULL) {
    if (weight != 0) {
      coco_error(
          "con_update_linear_combination(): gradient of constraint was zero");
    } else {
      coco_warning(
          "con_update_linear_combination(): gradient of constraint was zero");
    }
    ...
    if (data->x_shift_factor != 0)
      coco_warning("Inactive constraint passed to update_linear_combination, "
                   "x_shift_factor=%f",
                   data->x_shift_factor);
    if (weight == 0)
    ...
    if (weight < 0)
      coco_warning(
          "con_update_linear_combination: weight=%f < 0, should be > 0",
          weight);
    for (i = 0; i < problem->number_of_variables; ++i)
...
static coco_problem_t *c_linear_cons_bbob_problem_allocate(
  ...
  size_t inactive_constraints_left =
      number_of_active_constraints /
      2; /* TODO: decide whether this should go into the interface */
  ...
  number_of_linear_constraints =
      number_of_active_constraints + inactive_constraints_left;
  ...
  factor1 =
      global_scaling_factor * pow(10.0, coco_random_uniform(random_generator));
  ...
  problem_c = c_linear_single_cons_bbob_problem_allocate(
      function, dimension, instance, 1, factor1, problem_id_template,
      problem_name_template, gradient, 0.0, feasible_direction);
  if (gradient != NULL) /* preserve the constraint based on function gradient */
    first_constraint_data =
        (linear_constraint_data_t *)coco_problem_transformed_get_data(
            problem_c);
  else
  ...
  for (j = 2, i = 1; j <= number_of_linear_constraints; ++j) {
    ...
    if (i < number_of_active_constraints &&
        coco_random_uniform(random_generator) + 1e-23 >
            (double)inactive_constraints_left /
                (double)number_of_linear_constraints) { /* create an active
                                                           constraint */
      gradient = (++i - 1 + instance) % number_of_active_constraints
                     ? NULL
                     : gradient_c1;
      shift_factor = 0.0;
    } else { /* create an inactive (shifted) constraint */
      if (inactive_constraints_left-- <= 0)
        coco_error("c_linear_cons_bbob_problem_allocate(): no inactive left "
                   "i=%d j=%d nb_act=%ul nb_con=%ul",
                   i, j, number_of_active_constraints,
                   number_of_linear_constraints);
      gradient = NULL;
      shift_factor = 0.01 + 2.0 * coco_random_uniform(random_generator);
    }

    problem_c2 = c_linear_single_cons_bbob_problem_allocate(
        function, dimension, instance,
        i <= number_of_active_constraints
            ? i
            : number_of_linear_constraints - inactive_constraints_left,
        factor1, problem_id_template, problem_name_template, gradient,
        shift_factor, feasible_direction);
    if (shift_factor == 0) { /* active constraint */
      if (gradient !=
          NULL) { /* preserve the constraint based on function gradient */
        if (first_constraint_data)
          coco_warning(
              "c_linear_cons_bbob_problem_allocate(): first_constraint_data "
              "already assigned, this is probably a bug");
        first_constraint_data =
            (linear_constraint_data_t *)coco_problem_transformed_get_data(
                problem_c2);
      } else
        con_update_linear_combination(linear_combination, problem_c2,
                                      coco_random_uniform(random_generator2));
    }

    problem_c = coco_problem_stacked_allocate(
        problem_c, problem_c2, problem_c2->smallest_values_of_interest,
  ...
  if (number_of_active_constraints > 1 && disguise_gradient) {
    norm =
        first_constraint_data->gradient_norm; /* for reading convenience only */
    gradient = first_constraint_data->gradient; /* ditto */
    fac = coco_vector_scalar_product(linear_combination, gradient, dimension);
    if (fac < 0)
      coco_error("scalar product between first constraint and linear "
                 "combination = %f < 0 should be > 0",
                 fac);
    fac = fac <= 0 ? (double)dimension
                   : coco_double_min((double)dimension, norm * norm / fac);
    fac *= (1 + coco_random_uniform(random_generator2)) /
           (2 + 1. / (double)dimension); /* bound to <1 and >1/3 */
    for (i = 0; i < dimension; ++i)
    ...
    coco_vector_scale(gradient, dimension, norm,
                      fac); /* restore original norm */
  }
