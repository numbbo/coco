 ...
 *
 * Possible options:
 * - file_name: string (name of the output file; default value is
 * "first_hitting_times.dat")
 */
static void observer_toy(coco_observer_t *observer, const char *options,
  ...
  *option_keys = coco_option_keys_allocate(sizeof(known_keys) / sizeof(char *),
  ...
  observer_data =
      (observer_toy_data_t *)coco_allocate_memory(sizeof(*observer_data));

  /* Read file_name and number_of_targets from the options and use them to
  ...
  memcpy(file_name, observer->result_folder,
