 ...
 *
 * All of this code should only be used by the suite_bbob2009 functions to
 * provide compatibility to the legacy code. New test beds should strive to use
 * the new COCO facilities for random number generation etc.
 */
...
/**
 * @brief Allocates a n by m matrix structured as an array of pointers to double
 * arrays.
 */
...
/**
 * @brief Generates N uniform random numbers using inseed as the seed and stores
 * them in r.
 */
...
/**
 * @brief Converts from packed matrix storage to an array of array of double
 * representation.
 */
static double **bbob2009_reshape(double **B, double *vector, const size_t m,
                                 const size_t n) {
  size_t i, j;
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++) {
...
/**
 * @brief Generates N Gaussian random numbers using the given seed and stores
 * them in g.
 */
...
/**
 * @brief Computes a DIM by DIM rotation matrix based on seed and stores it in
 * B.
 */
static void bbob2009_compute_rotation(double **B, const long seed,
                                      const size_t DIM) {
 ...
 */
static void bbob2009_compute_xopt(double *xopt, const long seed,
                                  const size_t DIM) {
  size_t i;
...
/**
 * @brief Randomly chooses the objective offset for the given function and
 * instance.
 */
static double bbob2009_compute_fopt(const size_t function,
  ...
  if (function == 4)
    ...
    rseed = 17;
  else if (function == 101 || function == 102 || function == 103 ||
           function == 107 || function == 108 || function == 109)
    rseed = 1;
  else if (function == 104 || function == 105 || function == 106 ||
           function == 110 || function == 111 || function == 112)
    rseed = 8;
