 ...
 * decision values for the Gallagher function. The block-rotation is applied
 * only once per call and the result is stored such that the sub-problems can
 * access it
 *
 */
...
static void transform_vars_gallagher_blockrotation_free(void *thing) {
  transform_vars_gallagher_blockrotation_t *data =
      (transform_vars_gallagher_blockrotation_t *)thing;
 ...
 */
static void
transform_vars_gallagher_blockrotation_evaluate(coco_problem_t *problem,
  ...
  data = (transform_vars_gallagher_blockrotation_t *)
      coco_problem_transformed_get_data(problem);
  ...
  for (i = 0; i < inner_problem->number_of_variables; ++i) {
    ...
    for (j = versatile_data->first_non_zero_map[i];
         j < versatile_data->first_non_zero_map[i] +
                 versatile_data->block_size_map[i];
         ++j) {
      versatile_data->rotated_x[i] +=
          versatile_data->B[i][j - versatile_data->first_non_zero_map[i]] *
          x[j];
    }
    /*((f_gallagher_versatile_data_t *) problem->versatile_data)->rotated_x[i] =
     * x[i];*/
    data->x[i] = x[i]; /* to avoid pointer problems*/
  }

  coco_evaluate_function(
      inner_problem, data->x,
      y); /* does not modify the argument of the call since rotated_x will be
             used later in the sub_problems*/
  /* this function serves only to compute rotated_x on the problem level, not
   * for each sub-problem
   */
  assert(y[0] + 1e-13 >= problem->best_value[0]);
}

static COCO_UNUSED coco_problem_t *
transform_vars_gallagher_blockrotation(coco_problem_t *inner_problem);
...
transform_vars_gallagher_blockrotation(coco_problem_t *inner_problem) {
  ...
  data = (transform_vars_gallagher_blockrotation_t *)coco_allocate_memory(
      sizeof(*data));
  data->x = coco_allocate_vector(inner_problem->number_of_variables);

  if (coco_problem_best_parameter_not_zero(inner_problem)) {
    coco_debug("transform_vars_gallagher_blockrotation(): 'best_parameter' not "
               "updated, set to NAN");
  ...
  problem = coco_problem_transformed_allocate(
      inner_problem, data, transform_vars_gallagher_blockrotation_free,
      "transform_vars_gallagher_blockrotation");
