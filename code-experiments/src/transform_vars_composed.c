...
/**
 * @brief Inverse of composed non-linear transformation tcomp_uv obtained with
 * brentq.
 */
...
/**
 * @brief Applies the inverse of the composed transformation tasy to the initial
 * solution.
 *
 ...
 */
static void transform_inv_initial_composed(coco_problem_t *problem,
                                           const double *xopt) {
  size_t i;
  size_t j;
  int is_in_bounds;
  ...
  data_asy =
      (transform_vars_asymmetric_data_t *)coco_problem_transformed_get_data(
          inner_problem);
  data_osz =
      (transform_vars_oscillate_data_t *)coco_problem_transformed_get_data(
          problem);
  ...
  while (1) {
    ...
    for (i = 0; i < problem->number_of_variables; ++i) {
      dasy->i = i;
      di = tcomp_uv_inv(problem->initial_solution[i] *
