 ...
 * structures.
 */
static double f_step_ellipsoid_raw(const double *x,
  ...
  for (i = 0; i < number_of_variables; ++i) {
    ...
    data->x[i] = 0.0;
    c1 = sqrt(
        pow(condition / 10., (double)i / (double)(number_of_variables - 1)));
  ...
  result =
      0.1 * coco_double_max(fabs(x1) * 1.0e-4, result) + penalty + data->fopt;
...
static void f_step_ellipsoid_evaluate(coco_problem_t *problem, const double *x,
  ...
  y[0] = f_step_ellipsoid_raw(x, problem->number_of_variables,
                              (f_step_ellipsoid_data_t *)problem->data);
 ...
 */
static coco_problem_t *f_step_ellipsoid_bbob_problem_allocate(
  ...
  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "step ellipsoid function", f_step_ellipsoid_evaluate,
 ...
 * structures.
 */
static double f_step_ellipsoid_core(
    const double *x, const size_t number_of_variables,
 ...
 * an additional coordinate is added that will contain the value of \hat{z}_1
 * but that is ignored by functions other that f_step_ellipsoid_core and
 * transform_vars_round_step. The latter sets it.
 */
static coco_problem_t *
f_step_ellipsoid_allocate(const size_t number_of_variables) {

  coco_problem_t *problem = coco_problem_allocate_from_scalars(
      "step ellipsoid function", f_step_ellipsoid_permblock_evaluate, NULL,
      number_of_variables, -5.0, 5.0, 0.0);
  problem->versatile_data =
      (f_step_ellipsoid_versatile_data_t *)coco_allocate_memory(
          sizeof(f_step_ellipsoid_versatile_data_t));
  ((f_step_ellipsoid_versatile_data_t *)problem->versatile_data)->zhat_1 =
      0; /*needed for xopt evaluation*/
  /* add the free function of the allocated versatile_data*/
  problem->problem_free_function = f_step_ellipsoid_versatile_data_free;

  coco_problem_set_id(problem, "%s_d%02lu", "step_ellipsoid",
                      number_of_variables);
  /* Compute best solution, here done outside after the zhat is set to the
   * best_value */
  f_step_ellipsoid_permblock_evaluate(problem, problem->best_parameter,
                                      problem->best_value);
  return problem;
}
 ...
 */
static coco_problem_t *f_step_ellipsoid_permblockdiag_bbob_problem_allocate(
    const size_t function, const size_t dimension, const size_t instance,
    const long rseed, const char *problem_id_template,
    const char *problem_name_template) {
