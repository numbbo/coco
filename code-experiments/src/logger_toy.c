 ...
 *
 * Logs the evaluation number, function value the target hit and all the
 * variables each time a target has been hit.
 */
...
typedef struct {
  FILE *
      log_file; /**< @brief Pointer to the file already prepared for logging. */
 ...
 * outputs information based on the targets that have been hit.
 */
static void logger_toy_evaluate(coco_problem_t *problem, const double *x,
                                double *y) {

  logger_toy_data_t *logger =
      (logger_toy_data_t *)coco_problem_transformed_get_data(problem);
  size_t i;
  ...
  if (coco_observer_targets_trigger(logger->targets, y[0])) {
    fprintf(logger->log_file, "%lu\t%.*e\t%.*e",
            (unsigned long)logger->number_of_evaluations, logger->precision_f,
            y[0], logger->precision_f, logger->targets->value);
...
static coco_problem_t *logger_toy(coco_observer_t *observer,
  ...
  if (inner_problem->number_of_objectives != 1) {
    coco_warning("logger_toy(): The toy logger shouldn't be used to log a "
  ...
  problem = coco_problem_transformed_allocate(
      inner_problem, logger_data, logger_toy_free, observer->observer_name);
  ...
  fprintf(logger_data->log_file,
          "suite = '%s', problem_id = '%s', problem_name = '%s', coco_version "
          "= '%s'\n",
  ...
  fprintf(
      ...
      "%% evaluation number | function value | target hit | %lu variables \n",
