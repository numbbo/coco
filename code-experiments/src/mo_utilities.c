...
static double *mo_normalize(const double *y, const double *ideal, const double *nadir, const size_t num_obj) {
  ...
  for (i = 0; i < num_obj; i++) {
    ...
    if (normalized_y[i] < 0) {
      coco_warning("Adjusting normalized value of %.15e to the normalized value of %.15e", y[i], ideal[i]);
      normalized_y[i] = 0;
    }
  }
  ...
  for (i = 0; i < num_obj; i++) {
    ...
    if (coco_double_almost_equal(normalized_y[i], 0, mo_precision) && (normalized_y[1-i] < 1)) {
      coco_warning("Adjusting normalized value of %.15e to the normalized value of %.15e", y[1-i], nadir[1-i]);
      normalized_y[1-i] = 1;
    }
  }
