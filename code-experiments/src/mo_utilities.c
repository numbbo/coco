/**
 * @file mo_utilities.c
 * @brief Definitions of miscellaneous functions used for multi-objective
 * problems.
 */
 ...
 *
 * Two solutions are considered equal in objective space when their normalized
 * difference is smaller than mo_precision.
 *
...
/**
 * @brief Discretization interval used for rounding normalized multi-objective
 * solutions.
 *
...
/**
 * @brief Computes and returns the Euclidean norm of two dim-dimensional points
 * first and second.
 */
static double mo_get_norm(const double *first, const double *second,
                          const size_t dim) {
...
/**
 * @brief Creates a rounded normalized version of the given solution w.r.t. the
 * given ROI.
 *
 * If the solution seems to be better than the extremes it is corrected (2
 * objectives are assumed). The caller is responsible for freeing the allocated
 * memory using coco_free_memory().
 */
static double *mo_normalize(const double *y, const double *ideal,
  ...
  for (i = 0; i < num_obj; i++) {
    ...
    normalized_y[i] = coco_double_round(normalized_y[i] / mo_discretization) *
                      mo_discretization;
    if (normalized_y[i] < 0) {
...
/**
 * @brief Checks the dominance relation in the unconstrained minimization case
 * between two normalized solutions in the objective space.
 *
 * If two values are closer together than mo_precision, they are treated as
 * equal.
 *
 * @return
...
static int mo_get_dominance(const double *normalized_y1,
                            const double *normalized_y2, const size_t num_obj) {
  ...
  for (i = 0; i < num_obj; i++) {
    if (coco_double_almost_equal(normalized_y1[i], normalized_y2[i],
                                 mo_precision)) {
      continue;
...
/**
 * @brief Computes and returns the minimal normalized distance of the point
 * normalized_y from the ROI (equals 0 if within the ROI).
 *
