...
double HVCalculator::dSqrtDataNumber=0;
double HVCalculator::volume=0;
int HVCalculator::dataNumber=0;
int HVCalculator::dimension=0;    
...
double HVCalculator::computeHV(int dim, int n, std::vector<double> points, std::vector<double> refPoint){
    ...
    */
    ...
    sort(pointsInitial.begin(), pointsInitial.end(), HVCalculator::cmp);
...
bool HVCalculator::cmp(double* a, double* b) { 
	return (a[dimension-1] < b[dimension-1]);
}
bool HVCalculator::covers(const double* cub, const double regLow[]) {
	static int i;
...
bool HVCalculator::partCovers(const double* cub, const double regUp[]) {
	static int i;
	for (i=0; i<dimension-1; i++) {
		if (cub[i] >= regUp[i]) {
			return false;
...
int HVCalculator::containsBoundary(const double* cub, const double regLow[], const int split) {
...
double HVCalculator::getMeasure(const double regLow[], const double regUp[]) {
	static double vol;
	static int i;
	vol = 1.0;
	for (i=0; i<dimension-1; i++) {
...
int HVCalculator::isPile(const double* cub, const double regLow[], const double regUp[]) {
	static int pile;
	static int k;
...
double HVCalculator::getMedian(vector<double>& bounds) {
...
void HVCalculator::stream(double regionLow[], double regionUp[], const vector<double*>& points, int split, double cover) {
