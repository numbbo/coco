...
void coco_optimize(coco_problem_t *problem) { /* should at the least take budget as argument, but this is not coco_benchmark compliant */
  ...
  while ((remaining_budget = MAX_BUDGET - coco_problem_get_evaluations(problem)) > 0) {
    ...
    if (strcmp("mo_random_search", SOLVER_NAME) == 0) { /* random search for bi-objective case */
      mo_random_search(mo_objective_function, coco_problem_get_number_of_objectives(problem), dimension,
 ...
 */
/* Interface via dimension, function-ID and instance-ID. This does not translate directly to different
 * languages or benchmark suites. */
...
/**
 * Runs the tests (the same as run_experiments, except a smaller number of combinations is considered and the
 * created directories are removed in the end).
 */
static void run_tests(void) {
  ...
  coco_remove_directory(OBSERVER_OPTIONS);
}

/**
 * Runs the experiments unless called with "test" as command line argument.
 */
