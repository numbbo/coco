...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if(nrhs!=2) {
        mexErrMsgIdAndTxt("cocoGetSmallestValuesOfInterest:nrhs","Two inputs required.");
    }
    if(nlhs!=1) {
        mexErrMsgIdAndTxt("cocoGetSmallestValuesOfInterest:nlhs","One output required.");
    }
    ...
    if(strcmp(class_name, "Problem") != 0) {
        mexErrMsgIdAndTxt("cocoGetSmallestValuesOfInterest:notProblem","Input problem must be a Problem object.");
    }
    ...
    res = coco_get_smallest_values_of_interest(pb);
    /* free resources */
